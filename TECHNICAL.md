...
- `npm run agents:real-user-zero-to-autonomy` is the low-write real-backend startup gate. It starts `scripts/agent-project-server.mjs` with Secret Vault plus local mock model/search endpoints, seals model key plus search endpoint/key through `/secret-vault/seal`, proves `/search/test`, then drives product-team mission startup, provider-backed Agent evidence, brainstorm-board submission, model-backed product-brief draft, requested-changes review, linked revision-note, accepted final-deliverable, Manager Flow Graph, Readiness Proof Map, Product Team Delivery Trace, Group Chat transcript, timeline, event ledger, Agent Dashboard, and Evidence Index readiness through HTTP API calls without launching a browser or running a build.
- `npm run ui:real-user-zero-to-autonomy` is the stronger customer-facing startup gate. It starts the real `scripts/agent-project-server.mjs` process with Secret Vault plus local mock model/search endpoints; opens the built React app; seals model key plus search endpoint/key through Settings; proves `/search/test` returns provider evidence; then starts from Workspace Hub, runs kickoff, approves the mission, executes the C/A handoff intent from the Manager UI, records a provider-backed Agent evidence search through the same configured endpoint, and verifies the resulting brainstorm, evidence, model-backed product brief, review, revision, and accepted final-deliverable nodes are traceable through Manager Flow Graph, Readiness Proof Map, Group Chat transcript, timeline, event ledger, and Product Team Delivery Trace proof rows.
- `npm run ui:real-user-zero-to-autonomy:dev` runs that same browser script with `--ui-base-url=http://localhost:5173`, so an already running Vite dev server can be used as a lower-write preflight while replacing frontend mocks. It still starts the real backend and mock providers; it simply skips `vite build`, so it does not replace the built UI launch gate.
...
### 1.6 Collaboration Intent Queue
...
### 1.8 Autonomous Cycle Consistency
