...
- `npm run agents:product-team:research-sample` is the fast Research-as-validation-sample gate. It reuses the generic product-team backend and HTTP delivery trace, runs the three-step autonomous loop, and verifies Product Team Operating Loop plus Runtime Autonomy Status, then asserts the stage contract remains kickoff, self-marketing, brainstorm, evidence, draft, review/revision, final deliverable, proof surfaces, Manager continuation, Agent strategy, runtime recovery, and production blockers rather than paper/thesis/manuscript-specific protocol fields.
...
- `npm run ui:manager-mission-runner` is the focused C/A startup gate. It drives the browser through kickoff meeting, Manager clarification, Leader/next-action decisions, and real project approval through `POST /product-team-missions`, then verifies the reused-kickoff Mission Runner receipt, Autopilot tick, chat proof, timeline proof, and Manager Flow Graph mission node. Use it when changing initiation approval, Mission Runner receipts, proof exits, or frontend mock replacement around project startup.
- `npm run agents:real-user-zero-to-autonomy` is the low-write real-backend startup gate. It starts `scripts/agent-project-server.mjs` with Secret Vault plus local mock model/search endpoints, seals model key plus search endpoint/key through `/secret-vault/seal`, proves `/search/test`, then drives product-team mission startup, C/A handoff, direct Agent Autonomous Action Queue continuation, provider-backed Agent evidence, brainstorm-board submission, model-backed product-brief draft, requested-changes review, linked revision-note closure for every open change request, accepted final-deliverable, Manager Flow Graph, Readiness Proof Map, Product Team Delivery Trace, Group Chat transcript, timeline, event ledger, Agent Dashboard, and Evidence Index readiness through HTTP API calls without launching a browser or running a build. The gate fails if the handoff only produces control receipts and no substantive `agent-autonomous-action-run/v1` submission with autonomous action decision, chat, timeline, and event proof.
- `npm run ui:real-user-zero-to-autonomy` is the stronger customer-facing startup gate. It starts the real `scripts/agent-project-server.mjs` process with Secret Vault plus local mock model/search endpoints; opens the built React app; seals model key plus search endpoint/key through Settings; proves `/search/test` returns provider evidence; then starts from Workspace Hub, runs kickoff, approves the mission, executes the C/A handoff intent from the Manager UI, runs Agent Autonomous Action Queue against the same backend to prove post-handoff Agent initiative, records a provider-backed Agent evidence search through the same configured endpoint, closes all requested-change reviews with linked revisions, and verifies the resulting brainstorm, evidence, model-backed product brief, review, revision, and accepted final-deliverable nodes are traceable through Manager Flow Graph, Readiness Proof Map, Group Chat transcript, timeline, event ledger, and Product Team Delivery Trace proof rows.
...
Manager Dashboard also owns three coordination display contracts used by the main cockpit: `agent-management-mesh/v1` from `agents.managementMesh`, `assignment-timeline-matrix/v1` from `assignmentTimelineMatrix`, and `change-flow/v1` from `changeFlow`. React renders those rows with the same backend-backed/backend-required/demo source badge used by the routed read models. These panels are not new execution APIs; they are backend Dashboard facts for "who manages whom", "did an assignment become Agent work", and "did a change request reach owner work" so real projects do not fall back to browser-derived rows.

The main Manager Proof Map consumes `/projects/:id/readiness-proof-map` as `manager-proof-map/v1` by reading `readiness.checks` from the backend Proof Map. Offline/demo projects can still render the local `evaluateManagerScenarioReadiness` fallback, but backend-online real projects show a backend-required proof-map state when the route is absent or returns no checks.
