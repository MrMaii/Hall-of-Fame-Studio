...
`GET /projects/:id/evidence-source-review-workflow` exposes `evidence-source-review-workflow/v1`. It is the reviewer-facing governance layer above the raw source rows: the service derives one review item per source, attaches the reviewer handoff, quality/source-safety signals, local decision use, production decision use, proof route, gates, required production controls, and checksum. It can mark the local/private-pilot workflow ready when source safety, quality signals, reviewer handoff, and proof routes are complete, but it always keeps `readyForProduction: false` until human source-review approvals, calibrated source-quality policy, immutable source snapshots, provider receipts, and reviewer audit storage exist. Manager Ready Package embeds the workflow, `project-evidence-archive/v1` includes it in readiness models and manifest, pilot/production launch evidence routes link to it, and enforced access policy allows Manager/security-admin/observer reads.
...
- evidence source review workflow through `GET /projects/:id/evidence-source-review-workflow` and Manager Ready Package, returning `evidence-source-review-workflow/v1` with reviewer-visible source review items, review queue, quality/source-safety signals, proof routes, gates, production controls, and checksum. The archive manifest, pilot/production launch evidence routes, enforced access policy, product-team Harness, and Manager UI now include this route while keeping production source governance blocked
...
- real external search gateway adapters, calibrated source-quality policy, human source-review workflow, and managed provider/source audit storage
