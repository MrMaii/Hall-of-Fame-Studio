...
- **生产证据完整性审计**：Manager Ready Package 必须展示 `production-evidence-integrity-audit/v1`，后端必须通过 `GET /projects/:id/production-evidence-integrity-audit` 暴露同一份只读合同。该合同必须读取 operations、deployment、security 和 provider 四类生产控制 receipt，把每个 required control 的证据分为 missing、local rehearsal、external unattested 或 managed-production，展示 domain rows、control rows、proof ids、timeline/event ids、backend routes、summary 和 checksum。它必须进入 Manager Flow Graph、Readiness Proof Map、Security Boundary route policy、Manager UI 和产品团队 Harness，用来防止本地 rehearsal receipt 被误认为真实生产证书；只有全部 required controls 都带有明确 managed-production evidence 时才可以让该审计返回生产证据 ready
...
- `production-launch-audit/v1` must consume `production-evidence-integrity-audit/v1` as a first-class production gate named `managed-production-evidence-integrity`. Local/test receipts may verify control workflows, but they must keep this production gate failed until every required operations, deployment, security, and provider control points to explicit `managed-production` evidence.
- `production-launch-gap-register/v1` must also consume `production-evidence-integrity-audit/v1`. When managed-production evidence is missing, it must expose a routed `managed-production-evidence-integrity` gap pointing to `GET /projects/:id/production-evidence-integrity-audit`; when explicit managed-production receipts cover all required controls, that specific gap must close without claiming broader public-production readiness unless all other launch controls are complete.
