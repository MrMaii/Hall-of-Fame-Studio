...
`GET /local-runtime-health` is the compact operator entry point. It combines local store attachment, local-auth bootstrap state, telemetry error counts, scheduler state, secret-vault status, and the backup/recovery commands into one redacted `local-runtime-health/v1` response. When strict local authentication is enabled, it requires a local security-administrator session.
