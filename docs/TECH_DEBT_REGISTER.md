...
| TD-003 | 整个仓库 | 无单元测试：质量保障全靠 85+ 个 `scripts/validate-*.mjs`（静态源码断言 + Playwright 端到端） | 反馈慢（分钟级）、静态断言与实现强耦合（重构即碎）、无法隔离验证纯函数 | P0 | open | 引入 `node --test`（零新依赖）；优先给 modelProvider / secretVault / meeting 解析等纯逻辑补特征测试 |
...
| TD-005 | `scripts/agent-project-server.mjs` 与 `agentProjectService.js` | `providerApiKeyNames` / `providerEndpointNames` / `providerModelNames` / `normalizeProviderSecretTarget` / `providerSecretBindingForRecord` 两处完整复制 | 改一处忘另一处 → 密钥绑定行为不一致（已在本轮 diff 中出现两处同步修改） | P1 | open | 提取到 `src/agents/providerSecretBinding.js` 单一来源，两侧引用；行为不变 |
...
| TD-009 | 测试脚本 | `validate-local-mvp-release-checklist.mjs`（~4.1k 行）、`validate-frontend-mock-boundaries.mjs`（~3.1k 行）以字符串断言源码内容 | 与实现细节强耦合，源码合法重构也会打破校验；维护成本高 | P1 | open | 保留作为发布门禁，但新增测试一律走 node:test；拆分巨石时同步更新断言 |
...
| TD-012 | 行尾符 | 仓库内文件为 LF，Windows 工作区无 `.gitattributes`，git 每次操作都刷 CRLF 警告 | 噪音大；跨平台 diff 可能被行尾污染 | P2 | open | 添加 `.gitattributes`（`* text=auto eol=lf`）；纯元数据，不改代码 |
