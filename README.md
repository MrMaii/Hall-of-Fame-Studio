...
Initiation startup rule: Workspace Hub and Project Initiation both use `/local-mvp-startup-readiness`; the real kickoff and approval buttons stay disabled until the backend reports `readyForFirstProjectRun=true`, and the start/approval handlers repeat the same guard.
...
| `npm run agents:scenario:contract` | Low-write Manager scenario contract: Manager chat commands, Agent-to-Agent messages, Agent Workbench evidence/draft/review/revision/final-deliverable, timeline actions, Flow Graph, Proof Map, transcript, timeline, and event proof |
...
| `npm run agents:real-user-zero-to-autonomy-report` | Runs the same real backend zero-to-autonomy API path, verifies `GET /projects/:id/zero-to-autonomy-report`, and prints a redacted operator stage report with proof routes, generic artifact coverage, local/private readiness, and public-production blockers |
| `npm run agents:real-user-zero-to-autonomy-report:validate` | Validates that the project zero-to-autonomy report covers every required stage and artifact type without leaking provider secrets or claiming public-production readiness |
...
| `npm run launch:public-production:no-go` | Operator-facing P2 check for public-production readiness; reuses `launch:infra` and should currently return rehearsal proof plus explicit public-production no-go |
