...
For a lower-write API-only check of the same product-team loop, run:

```powershell
npm run agents:real-user-zero-to-autonomy
```

For a redacted stage report of the same backend path, run:
...
```bash
npm run agents:scenario:contract # Low-write Manager/Agent scenario contract validation
npm run agents:scenario   # Long end-to-end manager scenario regression
npm run agents:product-team:research-sample # Low-write Research-as-validation-sample gate for the generic product-team chain
npm run agents:mission-runner:startup # Low-write HTTP Mission Runner startup -> C/A handoff proof gate
npm run agents:mvp-readiness:operator-actions # Low-write HTTP C/A readiness action -> Agent output proof gate
npm run agents:agent-contract # Low-write The Pantheon Agent contract -> roster/Flow/Agent Dashboard proof gate
...
npm run skills:blend      # Persona docs + professional skill blending
...
`docs/LAUNCH_READINESS_GATES.md` is the operational launch gate matrix. It separates P0 local backend MVP, P1 customer private pilot, and P2 public production; lists the exact validation commands for each tier; and keeps public production blocked until managed persistence, queue/cron, BYOK/provider, security, operations, evidence export, and launch-governance controls are proven. `npm run launch:gates` verifies the matrix stays tied to runnable package scripts, while `npm run agents:production-access-control` verifies the focused signed-access, replay-protection, project-membership, Agent self-scope, fail-closed audit contract, and the operator-readable `GET /access-control-policy` contract before public traffic. `npm run agents:production-managed-identity` then verifies the focused managed identity and service identity startup contract, including signed managed-production identity evidence, redaction, and the operator-readable `GET /managed-identity-policy` contract. `npm run agents:production-managed-secrets` verifies the focused managed Secret Manager/KMS startup contract, including provider capability, rotation/access audit, attestation id/checksum, raw-secret blocking, redaction, and the operator-readable `GET /managed-secret-manager-policy` contract. `npm run agents:production-managed-persistence` verifies the focused managed database persistence startup contract, including real adapter requirement, database/gateway redaction, invalid signature blocking, signed managed-production cutover evidence, and the operator-readable `GET /managed-persistence-policy` contract. `npm run agents:production-managed-worker-queue` verifies the focused managed durable queue startup contract, including real queue/gateway configuration, queue endpoint redaction, scheduler/lease/retry/dead-letter requirements, invalid signature blocking, signed managed-production queue cutover evidence, and the operator-readable `GET /managed-worker-queue-policy` contract. `npm run agents:production-provider-controls` verifies the focused production provider/BYOK controls startup contract, including budget/rate policy, centralized usage audit, cost alert routing, invalid signature blocking, endpoint redaction, signed managed-production cost-control evidence, and the operator-readable `GET /production-provider-controls-policy` contract. `npm run agents:production-data-governance` verifies the focused production data governance startup contract, including retention policy, data residency, deletion job, export storage, invalid signature blocking, endpoint redaction, signed managed-production data-governance evidence, and the operator-readable `GET /production-data-governance-policy` contract. `npm run agents:production-traffic-controls` verifies the focused production traffic/rollback startup contract, including production domain/TLS, traffic gateway, health check, release approval, rollback smoke test, invalid signature blocking, endpoint redaction, signed managed-production traffic-control evidence, and the operator-readable `GET /production-traffic-policy` contract. `npm run agents:production-customer-acceptance` verifies the focused production customer acceptance startup contract, including customer policy, success criteria, threshold, human approval, rollback criteria, invalid signature blocking, signed managed-production customer-acceptance evidence, and the operator-readable `GET /production-customer-acceptance-policy` contract. `npm run agents:production-operations-startup` verifies the focused production operations startup contract, including centralized observability, alert routing, on-call ownership, managed incident records, restore drill proof, centralized audit retention, invalid signature blocking, endpoint redaction, signed managed-production operations evidence, and the operator-readable `GET /production-operations-policy` contract. `npm run launch:infra` runs the P2 infrastructure rehearsal for the public-production startup blocker contract, adapter gateway, reference private gateway server, Agent HTTP server env-gateway mode, and Postgres-compatible storage boundary. `npm run launch:public-production:no-go` is the operator-facing check for "can we open public traffic now"; it runs a dedicated no-go wrapper over the P2 infrastructure rehearsal, then checks public startup readiness and the redacted operator report still return explicit public-production no-go rather than production approval.
...
Active Portfolios now labels each project row by source: `backend-backed` when a backend catalog/detail/receipt has been seen, `sample-fixture` for explicit demo data, and `frontend-fallback` for browser-local cache rows. A fallback row is visible for inspection, but it is not a real backend project until the user syncs or creates it through backend routes.

Agent Focus Workbench also follows that boundary: backend-online real projects require the Agent Dashboard read model before task, review, submission, or evidence dependency options can be used as Agent-owned context. If the dashboard is missing, the Workbench shows a backend-required state, disables writes, and the write functions return a failed non-proof receipt instead of submitting from browser-local project rows.
...
The API fields in Settings Keys unlock as transient drafts after a backend URL is saved; backend `settings-provider-readiness/v1` separately controls whether `Seal` can persist those values through `/secret-vault/seal`. Provider secrets are never saved as browser-local configuration, and draft values stay in memory until Seal succeeds or the user leaves/edits the fields. The Keys tab now distinguishes unsynced status, unreachable backend status, and Vault-required setup, so a missing sync no longer looks like an absent API field or a fake local API store. `project-settings/v1` now also exposes `project-integration-capabilities/v1`, so Settings Integrations can show provider budget and Agent tool grants as backend-backed editable controls while Proxy/Webhook, MCP Tools, Vector Store, Budget Alerts, and Error Reporting are backend-backed read-only route contracts with production blockers instead of fake inputs or missing backend APIs. `GET /projects/:id/settings-integration-readiness` aggregates those rows as `settings-integration-readiness/v1`, embeds the result in Manager Ready Package as `settingsIntegrationReadiness`, and exposes `settingsIntegrationReadinessRoutes` in Readiness Proof Map so the Integrations tab is a backend readiness surface, not a route list or browser mock. It also exposes `project-workspace-policy/v1` plus `project-workspace-capabilities/v1`, so Settings Workspace can show global interface language as browser-local; project language, interface density, default visibility, autosave cadence, local workspace binding, runtime contract rules, long-term memory readiness, and meeting summaries as backend-backed instead of fake editable controls. The Workspace tab now binds a real local/private MVP filesystem workspace through `POST /projects/:id/workspace/bind` and reads it back through `GET /projects/:id/local-runtime`; production managed workspaces still require managed storage, access policy, quotas, and audit retention.
...
To make Settings provider entry usable in local development, start the backend with Secret Vault env before opening the UI: set `SECRET_VAULT_ENABLED=true`, set `SECRET_VAULT_KEY`, then run `npm run agents:server`. In PowerShell, the quickest local command is `$env:SECRET_VAULT_ENABLED='true'; $env:SECRET_VAULT_KEY='local-dev-vault-key'; npm run agents:server`. The Settings Keys tab shows the same command in its startup contract, stays backend-required, and disables sealing until `/secret-vault/status` is ready. When a Manager seals `model.apiKey`, `search.apiKey`, or `search.endpoint`, the backend binds that secret into the running provider as `local-secret-vault` instead of only writing a vault record. Model calls can become callable immediately when no policy block is configured. Search becomes callable only after a deterministic provider or a real `http-json` endpoint is configured; the UI can now seal the search endpoint and key through the same backend Vault route, then `/search/test` can verify the gateway. `agents:server` stores encrypted vault records in `.tmp/agent-secret-vault-records.json` by default, or in `SECRET_VAULT_RECORDS_FILE` when configured, so local restarts can reload sealed keys/endpoints as long as the same `SECRET_VAULT_KEY` is supplied. Run `npm run agents:server:validate` for the model-key startup path and `npm run agents:search-provider:vault-endpoint` for the search endpoint/key path.

`GET /local-mvp-startup-readiness` exposes `local-mvp-startup-readiness/v1`, the global backend startup preflight for local MVP use. It aggregates Secret Vault readiness, model/search runtime status, provider-vault redaction, project catalog readability, next action, and validation commands before the UI claims a user can begin a local product-team session. Workspace Hub now renders the same startup preflight beside **Start Initiation**, and Settings renders the same route in the Keys/Health flow. Manager Ready Package embeds `localMvpStartupReadiness`, and Readiness Proof Map exposes `localMvpStartupReadinessRoutes`, so first-run blockers are visible in the project proof surface. Run `npm run agents:local-mvp-startup-readiness` to verify the route moves from Vault-required to provider-setup-required without leaking provider secrets or claiming production readiness.

`GET /public-production-startup-readiness` exposes `public-production-startup-readiness/v1`, the global backend startup preflight for public production traffic. It checks enforced signed access, replay protection, audit fail-closed mode, managed Secret Manager/KMS, managed persistence, managed worker queue, provider redaction, adapter gateway attestation, production traffic/health/rollback controls, centralized observability, alert routing, on-call ownership, managed incidents, restore drill proof, and centralized audit retention. The same response now embeds `managed-secret-manager-readiness/v1`, which separates managed endpoint/key configuration, provider readiness, matched attestation id/checksum evidence, and raw-secret exposure; configured env names alone cannot promote the local AES-GCM vault to public-production ready. It also embeds `managed-infrastructure-cutover-readiness/v1`, which separates managed database/queue configuration, real-adapter requirement, local-shadow status, and managed-production cutover proof; configured database URLs or queue endpoints alone cannot approve public traffic. It also embeds `production-traffic-startup-readiness/v1`, which separates production domain/TLS, traffic gateway, health check, release approval, rollback smoke test, and signed managed-production traffic-control evidence; URL/env configuration alone cannot approve customer traffic. It also embeds `production-operations-startup-readiness/v1`, which separates observability, audit retention, incident response, and restore-drill configuration from signed managed-production evidence receipts; logs/metrics/traces, paging, incident, restore, audit endpoints, or unsigned receipt ids/checksums alone cannot approve public traffic. It also embeds `production-environment-setup/v1`, a redacted deployment setup matrix with domain rows, required env var names, missing groups, next action, route, and validation command, but no env values. The same backend response now embeds `public-production-action-plan/v1`, generated from blocked setup rows, and the redacted public-production operator report extends it with managed-environment preflight blockers. Settings runtime, Manager Ready Package, Readiness Proof Map, the project-level Production Launch Control Center, and the Manager UI link this route; Manager UI renders the setup matrix and action plan so operators can see the concrete missing production domains without opening raw JSON. Run `npm run agents:public-production-startup-readiness` to verify the current backend stays public-production blocked without leaking provider secrets.

The same public-production startup response now embeds `production-customer-acceptance-startup-readiness/v1`. That submodel requires customer production acceptance policy id, success criteria, acceptance threshold, approver role, approval id, rollback criteria, and signed managed-production customer acceptance receipt/checksum/signature evidence before the `customer-production-acceptance-policy` gate can pass. Private-pilot acceptance receipts or configured env var names alone still leave public production blocked.
...
Current Manager Dashboard replacement rule: real backend projects show Agent Current Work Status from `agent-state-summary/v1` task counts and current-task text; browser-local task lists are demo/offline fallback only.

Initiation fallback rule: if backend project approval fails, the normal user path does not create a local project. Explicit development fallback projects are tagged `development-fallback`, are not launch evidence, and are excluded from backend write commands.

Initiation startup rule: Workspace Hub and Project Initiation both use `/local-mvp-startup-readiness`; the real kickoff and approval buttons stay disabled until the saved/injected backend target reports `readyForFirstProjectRun=true`, and the start/approval handlers repeat the same guard. Unsaved Settings Deployment URL edits remain draft-only and are not used for hidden backend probes.
...
| `npm run agents:scenario:contract` | Low-write Manager scenario contract: Manager chat commands, Agent-to-Agent messages, marketplace Agent contract, Agent Workbench evidence/draft/review/revision/final-deliverable, timeline actions, Flow Graph, Proof Map, transcript, timeline, and event proof |
| `npm run agents:scenario` | Long manager demo data path regression |
...
| `npm run agents:settings-contracts` | Run the low-write aggregate Settings backend gate for local MVP startup, health, runtime, provider, search Vault endpoint/key, integration, privacy, budget, tool grants, integration capability, and workspace capability contracts |
...
| `npm run agents:transcript-contracts` | Low-write aggregate guard for backend Group Chat transcript search, channel pin, message pin, reply, mention, attachment, and member presence |
...
| `npm run agents:mission-runner:startup` | Focused low-write HTTP server gate for Product Team Mission Runner startup. It posts `/product-team-missions`, verifies kickoff self-marketing, Leader/Reviewer selection, `product-team-mission-run/v1`, C/A handoff, Runtime Autonomy Status, Collaboration Intent Queue handoff run, Agent submission output, Flow Graph, and Proof Map proof without running browser/build gates |
| `npm run agents:mvp-readiness:operator-actions` | Focused low-write HTTP server gate for the Manager C-side MVP readiness action handoff. It records `mvp-readiness-operator-action-run/v1`, projects the target into Collaboration Intent Queue and Autonomous Run Control, executes each generic core gap into Agent submissions/reviews, and verifies Flow Graph / Proof Map proof while keeping public production blocked |
| `npm run agents:product-team:local-mvp` | Low-write aggregate proof for the current local MVP product-team path: Settings/local startup contracts, Research-as-sample generic chain, Mission Runner HTTP startup/C/A handoff, marketplace Agent contract roster proof, Agent Workbench artifact/review/final-delivery proof, MVP readiness operator action into Agent output, zero-to-autonomy operator report, Private MVP Launch Package validation, and Manager Launch Operations visibility |
| `npm run agents:real-user-zero-to-autonomy` | Low-write real `agents:server` zero-to-autonomy gate: Secret Vault seal, provider tests, `/local-mvp-startup-readiness` first-project readiness, product-team mission, workspace bind/read/write proof, provider-backed evidence, Reviewer source decisions, all required generic artifact types from discovery through final deliverable, Artifact Quality Audit, ready Project Evidence Archive storage/workspace proof coverage, and proof surfaces through HTTP APIs |
| `npm run agents:real-user-zero-to-autonomy-report` | Runs the same real backend zero-to-autonomy API path, verifies `GET /projects/:id/zero-to-autonomy-report`, and prints a redacted operator stage report with proof routes, generic artifact coverage, search/model provider usage proof rows, provider receipt proof rows, Proof Map provider audit routes, local/private readiness, and public-production blockers |
| `npm run agents:real-user-zero-to-autonomy-report:validate` | Validates that the project zero-to-autonomy report covers every required stage, artifact type, provider usage proof, and provider receipt proof without leaking provider secrets or claiming public-production readiness |
| `npm run agents:private-mvp-launch-package` | Emits `private-mvp-launch-package/v1`, combining the zero-to-autonomy operator report with the public-production no-go report into one customer/investor-readable package for local MVP / controlled private-pilot rehearsal |
| `npm run agents:private-mvp-launch-package:validate` | Validates that the Private MVP Launch Package says local MVP and private-pilot rehearsal are ready while public production remains no-go, with no provider secret or vault ciphertext leakage |
| `npm run agents:launch-operations:private-mvp` | Verifies the backend `launch-operations-overview/v1` and Manager Ready Package expose the Private MVP Launch Package boundary, commands, public-production no-go status, Manager-readable Public Production Next Steps, and next-step run receipts for UI consumption |
| `npm run agents:product-team:core` | Stable low-write alias for the current local MVP product-team proof. It runs the same Settings/startup plus product-team aggregate as `agents:product-team:local-mvp` |
| `npm run agents:product-team:core:file-backed` | Longer file-backed/staged core runner for broader persistence, security, provider, and worker-queue coverage when the low-write local MVP proof is not enough |
| `npm run agents:product-team:research-sample` | Low-write Research-as-validation-sample gate. It reuses the generic in-memory product-team backend chain and asserts the proven stages remain kickoff, self-marketing, brainstorm, evidence, Reviewer source decisions, draft, review/revision, final deliverable, ready Project Evidence Archive handoff, and proof surfaces rather than paper/thesis/manuscript-specific protocol fields |
| `npm run agents:product-team:private-pilot:focused` | Low-write P1 backend focused chain: evidence handoff/export/download audit, provider eval, release candidate, controlled launch run, post-launch health, customer acceptance report, Flow Graph, and Proof Map without browser/build |
...
| `npm run launch:private-pilot:check` | One-command P1 release gate: runs the backend private-pilot acceptance chain and then the built Manager private-pilot browser handoff gate |
...
| `npm run ui:manager-backend:core:dev` | Lower-write preflight for the same C/A backend control-loop gate against an already running Vite dev server at `http://127.0.0.1:5173`; skips `vite build` and does not replace the built gate |
...
| `npm run ui:manager-backend:real-user-chain:dev` | Lower-write preflight for the same real-user Manager backend chain against an already running Vite dev server at `http://127.0.0.1:5173`; skips `vite build` and does not replace the built gate |
...
| `npm run ui:real-user-zero-to-autonomy` | Browser gate for one fresh user session on real `agents:server`: Settings Deployment backend target entry, Settings provider seal, Workspace and backend JSON startup readiness before Start Initiation, Settings Workspace local workspace bind/readback, kickoff approval, standalone Manager Dashboard C/A handoff before full Ready Package sync, delegated Agent Submission output, Reviewer source decisions, all required generic artifact types from discovery through final deliverable, Artifact Quality Audit, ready Project Evidence Archive storage/workspace proof readback, Flow Graph, Proof Map, transcript, timeline, event proof, and Manager UI readback |
...
| `npm run launch:local-mvp:check` | Low-write P0/P1 release checklist for script wiring, docs, canonical persona Skill bridge, and backend-first proof boundaries |
| `npm run agents:production-access-control` | Focused P2 access-control gate for enforced signed requests, replay rejection, project membership, Agent self-scope, observer write denial, persisted security audit proof, audit fail-closed behavior, and `GET /access-control-policy` operator contract coverage |
| `npm run agents:production-managed-secrets` | Focused P2 managed Secret Manager/KMS gate for provider capability, endpoint/key env groups, attestation pairs, raw-secret blocking, redaction, and `GET /managed-secret-manager-policy` operator contract coverage |
| `npm run agents:production-managed-identity` | Focused P2 managed identity gate for identity provider metadata, service identity boundaries, signed managed-production identity evidence, redaction, and continued public-production no-go |
| `npm run agents:production-managed-secrets` | Focused P2 managed Secret Manager/KMS gate for managed provider capability, rotation/access audit support, attestation id/checksum evidence, raw-secret blocking, redaction, and continued public-production no-go |
| `npm run agents:production-managed-persistence` | Focused P2 managed database persistence gate for real adapter configuration, database/gateway redaction, invalid cutover signature blocking, signed managed-production cutover evidence, `GET /managed-persistence-policy` operator contract coverage, and continued public-production no-go |
| `npm run agents:production-managed-worker-queue` | Focused P2 managed durable queue gate for real queue/gateway configuration, queue endpoint redaction, scheduler/lease/retry/dead-letter requirements, invalid cutover signature blocking, signed managed-production queue cutover evidence, `GET /managed-worker-queue-policy` operator contract coverage, and continued public-production no-go |
| `npm run agents:production-provider-controls` | Focused P2 provider/BYOK startup gate for production budget/rate policy, centralized usage audit, cost alert routing, redaction, signed managed-production cost-control evidence, `GET /production-provider-controls-policy` operator contract coverage, and continued public-production no-go |
| `npm run agents:production-data-governance` | Focused P2 data governance startup gate for retention policy, data residency, deletion job, export storage, redaction, signed managed-production data-governance evidence, `GET /production-data-governance-policy` operator contract coverage, and continued public-production no-go |
| `npm run agents:production-traffic-controls` | Focused P2 traffic/rollback startup gate for production domain/TLS, traffic gateway, health check, release approval, rollback smoke test, redaction, signed managed-production traffic-control evidence, `GET /production-traffic-policy` operator contract coverage, and continued public-production no-go |
| `npm run agents:production-customer-acceptance` | Focused P2 customer acceptance startup gate for customer policy, success criteria, threshold, human approval, rollback criteria, redaction, signed managed-production customer-acceptance evidence, `GET /production-customer-acceptance-policy` operator contract coverage, and continued public-production no-go |
| `npm run agents:production-operations-startup` | Focused P2 operations startup gate for centralized observability, alert routing, on-call ownership, managed incidents, restore drill, audit retention, redaction, signed managed-production operations evidence, `GET /production-operations-policy` operator contract coverage, and continued public-production no-go |
| `npm run launch:public-production:no-go` | Operator-facing P2 check for public-production readiness; runs the dedicated no-go wrapper over `launch:infra` and must currently return rehearsal proof plus explicit public-production no-go |
| `npm run ui:mock-boundaries` | Low-write static guard for frontend mock replacement: real backend projects must fail closed on missing transcripts/timeline/read models, sample/dev fallback must stay labeled, and browser seed paths must stay out of real projects |
...
| `npm run skills:check` | Validate 40 source persona packages plus deterministic routing regression cases |
| `npm run skills:blend` | Validate every registered persona has a loadable Skill document and persona + professional skill blending works |
