...
export function runAgentWorkCycle({
  ...
  const workText = task?.text || previousState.currentPlan?.focus || 'monitor direct obligations and keep the project channel current';
  ...
  const artifact = routine?.artifact || 'timeline evidence';
  const messageId = `agent_work_${agent.id}_${timestamp}`;
  const logId = `log_${messageId}`;
  const workSummary = completed
    ? `${agent.name} completed "${workText}" and published ${artifact} to the timeline.`
    : `${agent.name} advanced "${workText}" through ${routine?.label || 'their fixed work routine'} and published ${artifact} progress.`;
  const managementLine = resolvedManagementReasons.length ? ` Management priority: ${resolvedManagementReasons.join('; ')}.` : '';
  const managementResponseLine = managementSignals.length
    ? ` Responding to ${managementSignals.length} management signal${managementSignals.length === 1 ? '' : 's'} from ${managementResponderTargets.map((target) => target.name).join(' / ') || 'the management chain'}.`
    : '';
  const progressMessage = attachMessageReceipts({
    ...
    time: 'Agent Pulse',
    text: completed
      ? `Completed "${workText}". ${artifact} is on the project timeline.${managementLine}${managementResponseLine}`
      : `Progress on "${workText}": I ran ${routine?.label || 'my fixed routine'} and posted ${artifact} evidence to the timeline.${managementLine}${managementResponseLine}`,
    targets: [],
    weight: completed ? 'Completed' : 'Progress',
    agentWorker: {
      cycleId,
      agentId: agent.id,
      taskId: task?.id || null,
  ...
  const managementResponseMessages = managementResponderTargets.map((target, index) => attachMessageReceipts({
    ...
    role: agent.role,
    time: 'Agent Pulse',
    text: `${agent.name}: @${target.name} I picked up your management signal and folded it into "${workText}". Next proof marker is ${artifact}.`,
    targets: [target.name],
  ...
  const managementResponseLogs = managementResponderTargets.map((target, index) => ({
    ...
    agentId: agent.id,
    targetAgentId: target.id,
    log: `${agent.name} responded to ${target.name}'s management signal and folded it into the current Agent work pulse.`,
    cadence,
  ...
  const managementMessages = managementTargets.map((target, index) => {
    ...
    return attachMessageReceipts({
      ...
      role: agent.role,
      time: 'Agent Pulse',
      text: `${agent.name}: @${target.name} ${isPeerManaged ? 'peer-management' : 'management'} check-in from my Agent Pulse. Keep "${targetState.currentPlan?.focus || 'your current obligation'}" moving and post the next evidence marker.`,
      targets: [target.name],
      directTargetIds: [target.id],
      heardBy: [target.id],
      weight: isPeerManaged ? 'Peer Management' : 'Management',
      agentWorker: {
        cycleId,
        agentId: agent.id,
        targetAgentId: target.id,
  ...
  const managementLogs = managementTargets.map((target, index) => {
    ...
    return {
      ...
      agentId: agent.id,
      targetAgentId: target.id,
      log: `${agent.name} sent a ${isPeerManaged ? 'peer-management' : 'management'} check-in to ${target.name} from an independent Agent worker pulse.`,
      cadence,
  ...
  const nextState = {
    ...
    currentPlan: {
      ...
      focus: workText,
      next: completed ? 'wait for the next assignment or dependency' : 'continue work and publish the next proof pulse',
    ...
    worklog: [
      ...
      ...(managementSignals.length ? [{
        ...
        responseMessageIds: managementResponseMessages.map((message) => message.id),
        taskId: task?.id || null,
        text: `${agent.name} responded to ${managementSignals.length} management signal${managementSignals.length === 1 ? '' : 's'} before continuing the work pulse.`,
      }] : []),
...
export function createKickoffMeetingSession({
  ...
  const roleNegotiation = createKickoffRoleNegotiation(team, projectBrief, { projectId, projectName: name });
  const leaderElection = createLeaderElection(team, projectBrief, { projectId, projectName: name });
...
export function createKickoffProjectFromMeeting({
  ...
  const projectBrief = [name, brief].filter(Boolean).join(' ');
  const roleNegotiation = savedRoleNegotiation || createKickoffRoleNegotiation(team, projectBrief, { projectId, projectName: name });
  const leaderElection = savedLeaderElection || createLeaderElection(team, projectBrief, { projectId, projectName: name });
...
export function createAgentProjectService({
  ...
  return {
    ...
    getManagerDashboard(projectId) {
      return buildManagerDashboardSnapshot({
        project: store.getProject(projectId),
        messages: store.getMessages(projectId),
      });
    },
    getManagerCommandCenter(projectId) {
      return this.getManagerDashboard(projectId).managerCommandCenter;
    },
    runManagerCommandCenterNext({
      ...
      return {
        ...
        managerCommandCenter: this.getManagerCommandCenter(updatedProjectId),
        managerReadyPackage: this.getManagerReadyPackage(updatedProjectId),
      };
    },
    getManagerScenarioTrail(projectId) {
      return this.getManagerDashboard(projectId).managerScenarioTrail;
    },
    getManagerScenarioWalkthrough(projectId) {
      return this.getManagerDashboard(projectId).managerScenarioWalkthrough;
    },
    runManagerScenarioWalkthroughStep({
      ...
      return {
        ...
        resultInspection,
        managerScenarioWalkthrough: this.getManagerScenarioWalkthrough(projectAfterRun),
      };
    },
    getManagerRequirementMatrix(projectId) {
      return this.getManagerDashboard(projectId).managerRequirementMatrix;
    },
    getManagerUseCaseAudit(projectId) {
      return this.getManagerDashboard(projectId).managerUseCaseAudit;
    },
    getManagerActionQueue(projectId) {
      return this.getManagerDashboard(projectId).managerActionQueue;
    },
    runManagerActionQueueItem({
      ...
      return {
        ...
        managerActionQueue: this.getManagerActionQueue(result.project?.id || projectId),
      };
    },
    getManagerReadyPackage(projectId) {
      const managerDashboard = this.getManagerDashboard(projectId);
      return {
        projectId,
