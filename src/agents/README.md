...
- `getEvidenceSourceReviewWorkflow` and `GET /projects/:id/evidence-source-review-workflow`: export `evidence-source-review-workflow/v1`, the reviewer-facing source governance workflow derived from the evidence quality audit. It emits one source review item per source, reviewer handoff, review queue, source quality/source-safety signals, local/production decision use, proof routes, gates, required production controls, and checksum. Manager Ready Package embeds it, Project Evidence Archive includes it in readiness models and manifest, Pilot/Production launch evidence routes link to it, enforced access policy allows Manager/security-admin/observer reads, and production remains blocked until human source-review approvals, calibrated policy, source snapshots, provider receipts, and reviewer audit storage exist.
