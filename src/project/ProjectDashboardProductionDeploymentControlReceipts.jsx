export default function ProjectDashboardProductionDeploymentControlReceipts({
  onRecordReceipt,
  projectId,
  projectText,
  recordDisabled,
  ...
  const receiptRoute = workflow.backendRoutes?.productionDeploymentControlReceipts || `/projects/${projectId}/production-deployment-control-receipts`;
    ...
    <div data-testid="backend-production-deployment-control-receipts-snapshot" className="mt-3 border border-[#d8c99f] bg-[#efe2bd]/55 p-2">
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 md:justify-end">
          <span className={`node-status-tag ${workflow.readyForProductionDeployment ? 'bg-[#59684b] text-white' : 'bg-[#8f1e18] text-white'}`}>
            {workflow.readyForProductionDeployment ? projectText('deployment ready') : projectText('receipts needed')}
          </span>
          <button
            type="button"
            data-testid="backend-production-deployment-record-controls"
            onClick={() => onRecordReceipt({
              ...
              actorId: 'runtime-ops',
              reason: 'Record local rehearsal evidence for production deployment controls from the Manager UI.',
            ...
            disabled={recordDisabled || workflow.readyForProductionDeployment || !workflow.readyForPrivatePilotDeployment}
      ...
      <div className="mt-2 grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          ...
          [projectText('Private Pilot Deploy'), workflow.readyForPrivatePilotDeployment ? projectText('ready') : projectText('blocked')],
          [projectText('Control Proof'), workflow.readyForProductionDeploymentControls ? projectText('ready') : projectText('blocked')],
          [projectText('Production Deploy'), workflow.readyForProductionDeployment ? projectText('ready') : projectText('blocked')],
      ...
      <div className="mt-2 font-mono text-[8px] uppercase tracking-widest text-[#9b875c]">
        {projectText('Deployment receipts route')}: {receiptRoute}
      </div>
    </div>
  );
}
