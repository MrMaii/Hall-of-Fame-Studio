export default function ProjectDashboardProductionSecurityControlReceipts({
  onRecordReceipt,
  projectId,
  projectText,
  recordDisabled,
  ...
  const receiptRoute = workflow.backendRoutes?.productionSecurityControlReceipts || `/projects/${projectId}/production-security-control-receipts`;
    ...
    <div data-testid="backend-production-security-control-receipts-snapshot" className="mt-3 border border-[#d8c99f] bg-[#efe2bd]/55 p-2">
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 md:justify-end">
          <span className={`node-status-tag ${workflow.readyForProductionSecurity ? 'bg-[#59684b] text-white' : 'bg-[#8f1e18] text-white'}`}>
            {workflow.readyForProductionSecurity ? projectText('security ready') : projectText('receipts needed')}
          </span>
          <button
            type="button"
            data-testid="backend-production-security-record-controls"
            onClick={() => onRecordReceipt({
              ...
              reason: 'Record local rehearsal evidence for production security controls from the Manager UI.',
            ...
            disabled={recordDisabled || workflow.readyForProductionSecurity || !workflow.readyForLocalSecurityBoundary}
      ...
      <div className="mt-2 grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          ...
          [projectText('Local Boundary'), workflow.readyForLocalSecurityBoundary ? projectText('ready') : projectText('blocked')],
          [projectText('Control Proof'), workflow.readyForProductionSecurityControls ? projectText('ready') : projectText('blocked')],
          [projectText('Production Security'), workflow.readyForProductionSecurity ? projectText('ready') : projectText('blocked')],
      ...
      <div className="mt-2 font-mono text-[8px] uppercase tracking-widest text-[#9b875c]">
        {projectText('Security receipts route')}: {receiptRoute}
      </div>
    </div>
  );
}
