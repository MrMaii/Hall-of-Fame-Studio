...
test('complete Dashboard summary stays lazy and keeps its public proof controls', () => {
  ...
  for (const publicControl of [
    'project-dashboard-stat-source-',
    'project-dashboard-stat-source-detail-',
    'project-dashboard-next-recommendation-source',
    'project-dashboard-next-recommendation-source-detail',
    'project-dashboard-next-recommendation-sync-manager-dashboard',
    'dashboard-next-action-resolution',
  ]) {
  ...
  assert.ok(summarySource.includes('onSyncManagerDashboard'));
  assert.ok(summarySource.includes('managerNextSuggestion'));
});
