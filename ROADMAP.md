...
- Production launch gap register is exposed by `GET /projects/:id/production-launch-gap-register` and Manager Ready Package as `production-launch-gap-register/v1`. It normalizes production blockers from launch audit, deployment preflight, production operations, provider, evidence custody, artifact quality, and review governance into owner/domain/action rows with proof ids, timeline/event ids, upstream checksums, next action, Flow Graph node, Readiness Proof Map `productionLaunchGapRoutes`, Security Boundary route policy, Manager UI panel, and `npm run agents:product-team` coverage while keeping `readyForProduction: false`.
- Production launch control center is exposed by `GET /projects/:id/production-launch-control-center` and Manager Ready Package as `production-launch-control-center/v1`. It aggregates the launch audit, gap register, private-pilot go-live state, production operations receipts, production deployment receipts, production security receipts, production provider receipts, launch approvals, deployment preflight, provider readiness, security boundary, custody, and artifact quality into release gate rows, owner rows, stage rows, next action, proof ids, timeline/event ids, upstream checksums, Flow Graph node, Readiness Proof Map `productionLaunchControlCenterRoutes`, Security Boundary route policy, Manager UI panel, and product-team Harness coverage while keeping public production `no-go`.
