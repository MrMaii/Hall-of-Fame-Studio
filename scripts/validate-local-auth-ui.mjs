...
try {
  ...
  await page.getByTestId('first-run-username').fill('owner');
  await page.getByTestId('first-run-display-name').fill('Local Owner');
  await page.getByTestId('first-run-password').fill('correct horse battery staple1');
  await page.getByTestId('first-run-auth-submit').click();
  await page.getByTestId('open-settings-button').waitFor({ state: 'visible', timeout: 15_000 });
