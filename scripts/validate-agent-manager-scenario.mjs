...
assert(appSource.includes('roomTranscript.slice(-8).map') && appSource.includes('backend-kickoff-opening-turn'), 'Initiation meeting UI must render runtime-generated meeting transcript entries.');
...
assert(appSource.includes('selectMeetingLeaderCandidate') && appSource.includes('/leader') && appSource.includes('Kickoff meeting Leader confirmed through backend') && appSource.includes('leader-candidate-'), 'Real initiation meeting must let managers persist the Leader confirmation during the campaign stage.');
...
assert(appSource.includes('saveInitiationNextActionsToMeeting') && appSource.includes('initiation-next-action-') && appSource.includes('initiation-add-next-action') && appSource.includes('/next-actions') && appSource.includes('Kickoff meeting next actions confirmed through backend'), 'Real initiation meeting must let managers persist first execution action resolution during the meeting.');
...
assert(appSource.includes('initiationConfirmedTeamIds') && appSource.includes('toggleConfirmedTeamMember') && appSource.includes('Removed after meeting'), 'Real initiation meeting must let managers finalize the project team during the meeting.');
...
assert(appSource.includes('if (!activeProject || !agentId || !shouldAttemptBackendProjectWrite(activeProject)) return;') && appSource.includes('if (!activeProject?.id || !submission?.id || !shouldAttemptBackendProjectWrite(activeProject)) return;') && appSource.includes('const workbenchWriteDisabled = !backendCommandAvailable || backendStation.loading || workbenchBackendContextMissing;') && appSource.includes('disabled={workbenchWriteDisabled || workbenchTaskOptions.length === 0}') && appSource.includes('disabled={workbenchWriteDisabled || workbenchReviewOptions.length === 0}') && appSource.includes('disabled={workbenchWriteDisabled}') && appSource.includes('disabled={!backendCommandAvailable || backendStation.loading}') && appSource.includes('disabled={!backendCommandAvailable || backendStation.loading || !rowReviewerId}') && appSource.includes('disabled={!backendCommandAvailable || backendStation.loading || !selectedMessageTarget}'), 'Agent Workbench, Reviewer composer, Agent Pulse, and Agent-to-Agent message controls must remain backend-attemptable from the configured backend command target while Workbench also fails closed when a backend-required Agent Dashboard is missing.');
