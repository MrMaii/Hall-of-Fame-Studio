...
function cleanupManagerScenarioTmp() {
  ...
  for (const fileName of managerScenarioTmpFiles) {
    ...
    rmSync(`${filePath}.provider-rate-limits.json`, { force: true });
...
const scenarioWatchdogTimeoutMs = Number(process.env.HOFS_MANAGER_SCENARIO_TIMEOUT_MS || 420000);
...
const agentAutonomousActionQueueUiSource = readFileSync(new URL('../src/project/ProjectDashboardAgentAutonomousActionQueue.jsx', import.meta.url), 'utf8');
const projectComponentSource = readdirSync(new URL('../src/project/', import.meta.url))
  .filter(name => /\.jsx$/.test(name))
  .sort()
  .map(name => readFileSync(new URL(`../src/project/${name}`, import.meta.url), 'utf8'))
  .join('\n');
const onboardingComponentSource = readdirSync(new URL('../src/onboarding/', import.meta.url))
  .filter(name => /\.jsx$/.test(name))
  .sort()
  .map(name => readFileSync(new URL(`../src/onboarding/${name}`, import.meta.url), 'utf8'))
  .join('\n');
const projectUiSource = `${appSource}\n${projectComponentSource}\n${onboardingComponentSource}`;
const advancedMeetingRoomSource = readFileSync(new URL('../src/meeting/AdvancedMeetingRoom.jsx', import.meta.url), 'utf8');
const meetingUiSource = `${appSource}\n${advancedMeetingRoomSource}`;
const advancedProjectChatSource = readFileSync(new URL('../src/project/AdvancedProjectChat.jsx', import.meta.url), 'utf8');
const chatUiSource = `${appSource}\n${advancedProjectChatSource}`;
const advancedProjectTimelineSource = readFileSync(new URL('../src/project/AdvancedProjectTimeline.jsx', import.meta.url), 'utf8');
const timelineUiSource = `${appSource}\n${advancedProjectTimelineSource}`;
const advancedWorkspaceViewSource = readFileSync(new URL('../src/workspace/AdvancedWorkspaceView.jsx', import.meta.url), 'utf8');
const workspaceUiSource = `${appSource}\n${advancedWorkspaceViewSource}`;
const projectDashboardHeaderSource = readFileSync(new URL('../src/project/ProjectDashboardHeader.jsx', import.meta.url), 'utf8');
const projectDashboardAgentOverviewSource = readFileSync(new URL('../src/project/ProjectDashboardAgentOverview.jsx', import.meta.url), 'utf8');
const projectDashboardManagerActionPlaybookSource = readFileSync(new URL('../src/project/ProjectDashboardManagerActionPlaybook.jsx', import.meta.url), 'utf8');
const projectDashboardManagerActionRunLedgerSource = readFileSync(new URL('../src/project/ProjectDashboardManagerActionRunLedger.jsx', import.meta.url), 'utf8');
const projectDashboardManagerScenarioTrailSource = readFileSync(new URL('../src/project/ProjectDashboardManagerScenarioTrail.jsx', import.meta.url), 'utf8');
const projectDashboardManagerScenarioWalkthroughSource = readFileSync(new URL('../src/project/ProjectDashboardManagerScenarioWalkthrough.jsx', import.meta.url), 'utf8');
const projectDashboardSyncProtocolAuditSource = readFileSync(new URL('../src/project/ProjectDashboardSyncProtocolAudit.jsx', import.meta.url), 'utf8');
...
assert(meetingUiSource.includes('roomTranscript.slice(-8).map') && appSource.includes('backend-kickoff-opening-turn'), 'Initiation meeting UI must render runtime-generated meeting transcript entries.');
assert(meetingUiSource.includes('Heard by'), 'Initiation meeting UI must expose which Agents heard each transcript turn.');
...
assert(chatUiSource.includes('focusedChatProofIds') && chatUiSource.includes('Proof focus:'), 'Chat proof navigation must highlight exact evidence messages.');
assert(timelineUiSource.includes('focusedTimelineProofIds') && timelineUiSource.includes('Timeline proof focus'), 'Timeline proof navigation must highlight exact evidence logs.');
assert(chatUiSource.includes('data-chat-proof-id') && appSource.includes('scrollIntoView'), 'Chat proof navigation must auto-scroll to the exact evidence message.');
assert(timelineUiSource.includes('data-timeline-event-id') && appSource.includes('timelineViewportRef'), 'Timeline proof navigation must auto-pan toward the exact evidence node.');
assert(timelineUiSource.includes('timeline-evidence-detail') && timelineUiSource.includes('Source Channel') && timelineUiSource.includes('Receipts') && timelineUiSource.includes('Direct Targets') && appSource.includes('directTargetIds: log.directTargetIds'), 'Timeline detail must expose channel, receipt, and direct-target evidence for runtime events.');
assert(projectUiSource.includes('Group Chat Transcript Index') && projectUiSource.includes('group-chat-transcript-index') && projectUiSource.includes('Message Count') && projectUiSource.includes('Archived Proofs') && projectUiSource.includes('Latest Speaker') && projectUiSource.includes('Receipt Coverage') && projectUiSource.includes('Direct Mentions'), 'Project dashboard must expose a group-chat transcript index with counts, archived proof recovery, latest speaker, receipts, and direct mentions.');
assert(projectUiSource.includes('channelTranscriptRows') && projectUiSource.includes('projectTranscriptMessages') && projectUiSource.includes('recoveredProofIdsByChannel') && projectUiSource.includes('Open transcript') && projectUiSource.includes('transcript-channel-'), 'Group Chat Transcript Index must derive project-scoped channel rows, recover archived proof ids, and open exact transcript proof.');
assert(projectUiSource.includes('syncBackendProjectTranscripts') && projectUiSource.includes('/transcripts') && projectUiSource.includes('transcriptIndex') && projectUiSource.includes('transcriptChannels') && projectUiSource.includes('backend-sync-transcripts') && projectUiSource.includes('Sync transcripts') && projectUiSource.includes('backendTranscriptReadModelMissing') && projectUiSource.includes('backend-transcript-index-required') && projectUiSource.includes('browser-local chat recovery is suppressed'), 'Group Chat Transcript Index must consume backend transcript read models when online, show backend-required missing state, and avoid treating browser chat history as the only collaboration source.');
...
assert(appSource.includes("requestAgentBackend('/kickoff-meetings'") && appSource.includes('initiationMeetingSession') && meetingUiSource.includes('initiation-meeting-session-proof'), 'Real initiation flow must create a durable kickoff meeting session before approval.');
...
assert(projectUiSource.includes('backend-product-team-mission-runs-snapshot') && projectUiSource.includes('backendProductTeamMissionRuns') && projectUiSource.includes('backendLatestProductTeamMissionRun') && projectUiSource.includes('Mission route:'), 'Backend Manager Snapshot must render Product Team Mission Runner receipts and routes after initiation approval.');
assert(projectUiSource.includes('backend-product-team-mission-chat-proof') && projectUiSource.includes('backend-product-team-mission-timeline-proof') && projectUiSource.includes('backend-product-team-mission-flow-node') && serviceSource.includes('meeting.evidence?.roleTranscriptIds'), 'Backend Manager Snapshot must let the Manager jump from Mission Runner receipt to chat proof, timeline proof, and the Flow Graph mission node.');
...
assert(projectUiSource.includes('initiation-meeting-generation-source') && projectUiSource.includes('initiation-result-generation-source') && projectUiSource.includes('validation fallback') && projectUiSource.includes('provider-backed model'), 'Initiation UI must expose kickoff generation provenance instead of hiding deterministic/model fallback status.');
assert(projectUiSource.includes('kickoff-dashboard-generation-source') && appSource.includes('kickoffGenerationProvenance') && projectUiSource.includes('Kickoff Generation Source'), 'Project dashboard must keep kickoff generation provenance visible after approval, not only during initiation.');
...
assert(projectUiSource.includes('initiation-backend-error'), 'Backend-connected initiation failures must be visible in the initiation flow instead of silently creating a local project.');
...
assert(workspaceUiSource.includes('start-initiation-button') && projectUiSource.includes('initiation-approve-create'), 'Real initiation flow must expose stable UI hooks for backend-connected approval validation.');
assert(projectUiSource.includes('initiation-director-decisions') && projectUiSource.includes('Confirmed Team') && projectUiSource.includes('Confirmed Leader Marker') && projectUiSource.includes('First Execution Plan') && projectUiSource.includes('leader-candidate-'), 'Real initiation result must expose explicit Director decisions for team, Leader marker, and next execution plan.');
assert(projectUiSource.includes('selectMeetingLeaderCandidate') && projectUiSource.includes('/leader') && projectUiSource.includes('Kickoff meeting Leader confirmed through backend') && projectUiSource.includes('leader-candidate-'), 'Real initiation meeting must let managers persist the Leader confirmation during the campaign stage.');
assert(projectUiSource.includes('initiationActionDrafts') && projectUiSource.includes('initiation-next-action-') && projectUiSource.includes('initiation-add-next-action'), 'Real initiation result must let managers edit first execution next actions before approval.');
assert(projectUiSource.includes('saveInitiationNextActionsToMeeting') && projectUiSource.includes('initiation-next-action-') && projectUiSource.includes('initiation-add-next-action') && projectUiSource.includes('/next-actions') && projectUiSource.includes('Kickoff meeting next actions confirmed through backend'), 'Real initiation meeting must let managers persist first execution action resolution during the meeting.');
assert(appSource.includes('submitInitiationClarification') && appSource.includes('meetingRoleQuestions') && appSource.includes('role questions answered') && appSource.includes('selectedClarificationQuestion') && appSource.includes('/clarify'), 'Real initiation meeting must let managers answer specific Agent role questions before approval.');
assert(appSource.includes('const submitInitiationMeetingInput = async') && appSource.includes('Kickoff meeting turn continued through backend model') && appSource.includes('Initiation meeting input must be attached to a backend kickoff meeting session.') && appSource.includes('Browser-local meeting response fallback was suppressed.') && !appSource.includes('runRoomSimulation(text, meetingProject);'), 'Initiation meeting free input must persist as backend kickoff clarification instead of generating local mock meeting turns.');
assert(projectUiSource.includes('initiationConfirmedTeamIds') && projectUiSource.includes('toggleConfirmedTeamMember') && projectUiSource.includes('Removed after meeting'), 'Real initiation meeting must let managers finalize the project team during the meeting.');
...
assert(projectUiSource.includes('kickoffCharterProofIds') && projectUiSource.includes('Kickoff chat proof'), 'Kickoff charter must offer a direct jump back to the kickoff group chat evidence.');
assert(projectUiSource.includes('kickoff-brief-alignment') && projectUiSource.includes('Kickoff Brief Alignment') && projectUiSource.includes('Project Brief') && projectUiSource.includes('Brief Heard By') && projectUiSource.includes('Brief proof') && projectUiSource.includes('Role response proof'), 'Project dashboard must expose Director brief alignment before role questions and self-nominations.');
assert(projectUiSource.includes('kickoff-confirmed-team-matrix') && projectUiSource.includes('Confirmed Team Matrix') && projectUiSource.includes('Director-selected roster persisted to project state and kickoff charter') && projectUiSource.includes('Team timeline proof'), 'Project dashboard must expose the Director-confirmed team roster with project-state, charter, and timeline proof.');
assert(projectUiSource.includes('kickoff-meeting-flow') && projectUiSource.includes('Kickoff Meeting Flow') && projectUiSource.includes('Role Clarification') && projectUiSource.includes('Self Nominations') && projectUiSource.includes('Peer Hearing') && projectUiSource.includes('Leader Campaign') && projectUiSource.includes('Director Confirmation') && projectUiSource.includes('Leader Marker'), 'Project dashboard must expose the kickoff meeting flow from role clarification through Director-confirmed Leader marker.');
assert(projectUiSource.includes('kickoffMeetingFlow') && projectUiSource.includes('roleHearingCount') && projectUiSource.includes('leaderHearingCount') && projectUiSource.includes('leaderElectionResolution') && projectUiSource.includes('leaderMarkerPersisted') && projectUiSource.includes('Kickoff meeting proof'), 'Kickoff Meeting Flow must derive peer-hearing, Leader election resolution, and Leader-marker proof from project state and offer a chat proof jump.');
...
assert(projectUiSource.includes('kickoff-conversation-flow') && projectUiSource.includes('Conversation Evidence') && projectUiSource.includes('Conversation proof'), 'Kickoff Meeting Flow must expose concrete conversation rows and proof jumps.');
assert(projectUiSource.includes('kickoff-hearing-matrix') && projectUiSource.includes('Kickoff Hearing Matrix') && projectUiSource.includes('Role Questions Heard') && projectUiSource.includes('Self Nominations Heard') && projectUiSource.includes('Leader Campaign Hearing') && projectUiSource.includes('Heard By:') && projectUiSource.includes('Hearing proof'), 'Kickoff Meeting Flow must expose a hearing matrix that maps every role question, self-nomination, and Leader campaign turn to peer receipts and proof jumps.');
...
assert(projectUiSource.includes('Kickoff Execution Flow') && projectUiSource.includes('Meeting decisions to first 24/7 work pulse') && projectUiSource.includes('Next Action Resolution') && projectUiSource.includes('kickoff-next-action-resolution') && projectUiSource.includes('kickoff-next-action-agent-receipts') && projectUiSource.includes('Agent receipts:') && projectUiSource.includes('First pulse chat proof') && projectUiSource.includes('First pulse timeline proof'), 'Project dashboard must expose kickoff execution flow from confirmed next actions to Agent receipts and first autonomous pulse.');
assert(projectUiSource.includes('all-agent-startup-matrix') && projectUiSource.includes('All-Agent Startup Matrix') && projectUiSource.includes('Every confirmed Agent enters a fixed routine') && projectUiSource.includes('Startup timeline proof') && projectUiSource.includes('Startup Agents') && projectUiSource.includes('Routine Plan') && projectUiSource.includes('Startup Proof'), 'Project dashboard must expose all-Agent startup, fixed routine, and next-run queue proof after kickoff approval.');
...
assert(projectUiSource.includes('changeTimelineProofIds') && projectUiSource.includes('Change chat proof') && projectUiSource.includes('Change timeline proof'), 'Change Ledger must offer direct chat and timeline proof jumps.');
assert(projectUiSource.includes('change-stage-') && projectUiSource.includes('Source Request') && projectUiSource.includes('Team Discussion') && projectUiSource.includes('Owner Confirmation') && projectUiSource.includes('Owner Plan') && projectUiSource.includes('Team Sync') && projectUiSource.includes('change.confirmationMessageId') && projectUiSource.includes('change.syncMessageId'), 'Change Ledger must visibly stage source request, team discussion, owner confirmation, owner plan, and team sync evidence.');
assert(projectUiSource.includes('changeFlowRows') && projectUiSource.includes('ownerState.currentPlan?.changeRecordId') && projectUiSource.includes('Sync Targets') && projectUiSource.includes('change-sync-targets-') && projectUiSource.includes('change-discussion-receipts-') && projectUiSource.includes('Discussion receipts:'), 'Change Ledger must prove owner-plan linkage, synchronized Agents, and discussion receipt coverage.');
assert(projectUiSource.includes('change-resolution-matrix') && projectUiSource.includes('Change Resolution Matrix') && projectUiSource.includes('Source Intake') && projectUiSource.includes('Owner First Work') && projectUiSource.includes('Owner work chat proof') && projectUiSource.includes('Owner work timeline proof'), 'Project dashboard must expose a change resolution matrix from intake through owner first work proof.');
assert(projectUiSource.includes('ownerWorkStarted') && projectUiSource.includes('ownerWorkMessageIds') && projectUiSource.includes('ownerWorkTimelineIds') && projectUiSource.includes('Change Owner Pulses'), 'Change rows must derive owner first-work pulse proof and expose it in the backend manager snapshot.');
...
assert(projectUiSource.includes('dual-channel-change-intake-matrix') && projectUiSource.includes('Dual-channel Change Intake Matrix') && projectUiSource.includes('Source Message') && projectUiSource.includes('Source Receipts') && projectUiSource.includes('Team Discussed') && projectUiSource.includes('Source channel proof') && projectUiSource.includes('Resolution chat proof') && projectUiSource.includes('Change Intake'), 'Project dashboard must expose War Room and Google Chat change intake receipts before owner resolution.');
...
assert(projectUiSource.includes('dual_channel_change') && projectUiSource.includes('runMultiChannelChangeBroadcast') && projectUiSource.includes('Broadcast dual-channel change'), 'Manager demo must expose a one-click meeting plus Google Chat change broadcast.');
...
assert(projectUiSource.includes('handoffTimelineProofIds') && projectUiSource.includes('Peer chat proof') && projectUiSource.includes('Peer timeline proof'), 'Peer Handoffs must offer direct chat and timeline proof jumps.');
assert(projectUiSource.includes('managementEventCount') && projectUiSource.includes('task proof link'), 'Autonomous Work Loop must surface ongoing Agent management check-ins.');
assert(projectUiSource.includes('24/7 Operations Board') && projectUiSource.includes('operations-board-24-7') && projectUiSource.includes('Project Next Run') && projectUiSource.includes('Project Last Run') && projectUiSource.includes('Backend Worker') && projectUiSource.includes('Agent Run Queue'), 'Project dashboard must expose a 24/7 Operations Board with project cadence, backend worker, and Agent run queue evidence.');
assert(projectUiSource.includes('operationsBoardRows') && projectUiSource.includes('Next Agent Run') && projectUiSource.includes('Latest Agent Work') && projectUiSource.includes('Worker Trigger') && projectUiSource.includes('Management Priority') && projectUiSource.includes('operations-agent-'), '24/7 Operations Board must show each Agent next run, latest work, trigger, and management priority.');
assert(projectUiSource.includes('continuous-work-loop') && projectUiSource.includes('Continuous Work Loop') && projectUiSource.includes('Scheduler to Agent pulse to timeline proof') && projectUiSource.includes('Run Loop Pulse') && projectUiSource.includes('Loop chat proof') && projectUiSource.includes('Loop timeline proof'), 'Project dashboard must expose a continuous 24/7 work-loop view from scheduler to Agent pulse to chat and timeline proof.');
assert(projectUiSource.includes('continuousWorkRows') && projectUiSource.includes('loopState') && projectUiSource.includes('proofReady') && projectUiSource.includes('Continuous Rows') && projectUiSource.includes('Continuous Proofs'), 'Continuous Work Loop must derive per-Agent loop state, proof readiness, and backend manager snapshot counts.');
...
assert(projectUiSource.includes('Fixed Work Routines') && projectUiSource.includes('routine-row-') && projectUiSource.includes('Routine Checklist') && projectUiSource.includes('Next Evidence'), 'Project dashboard must expose every Agent fixed routine as a manager-visible matrix.');
assert(projectUiSource.includes('Agent Management Mesh') && projectUiSource.includes('agent-management-mesh') && projectUiSource.includes('Leader Chain') && projectUiSource.includes('Managed Agents') && projectUiSource.includes('Latest Check-in') && projectUiSource.includes('Management Proof'), 'Project dashboard must expose Agent-to-Agent management relationships and check-in proof.');
assert(projectUiSource.includes('managementMeshRows') && projectUiSource.includes('managementLogTypes') && projectUiSource.includes('peer-management-check-in') && projectUiSource.includes('management-response') && projectUiSource.includes('Management timeline proof'), 'Agent Management Mesh must derive leader/peer-management response proof from Agent state and timeline logs.');
assert(projectUiSource.includes('Peer Management Matrix') && projectUiSource.includes('peer-management-matrix') && projectUiSource.includes('Every independent Agent has a peer manager and a peer target'), 'Agent Management Mesh must expose the explicit peer-management matrix.');
assert(projectUiSource.includes('runBackendManagementSync') && projectUiSource.includes('manager-ui-management-sync') && projectUiSource.includes('management-sync') && projectUiSource.includes('agent-management-sync-') && projectUiSource.includes('Run Management Sync'), 'Agent Management Mesh must let managers explicitly trigger a backend-backed Agent management sync from the relationship graph.');
...
assert(projectUiSource.includes('attachMessageReceipts') && projectUiSource.includes('Seen {message.visibility.receiptCount}') && projectUiSource.includes('Heard by {receipts.heardText}') && projectUiSource.includes('Direct target {receipts.directText}'), 'Group chat must surface durable message receipt counts and named recipients.');
assert(projectUiSource.includes('Unified Event Ledger') && projectUiSource.includes('activeProject.eventLedger'), 'Project dashboard must surface the unified project event ledger.');
assert(projectUiSource.includes('summary: eventLedgerSummary') && projectUiSource.includes('Retained {summary.retainedCount}'), 'Project dashboard must expose retained/total event-ledger projection.');
assert(projectUiSource.includes('const replayProjection = summary.replayProjection || {};') && projectUiSource.includes("['Handoff', replayProjection.peerHandoffCount]"), 'Project dashboard must expose event-ledger replay projection counts.');
assert(projectUiSource.includes('syncBackendTimelineAndEvents') && projectUiSource.includes('/timeline') && projectUiSource.includes('/events') && projectUiSource.includes('timelineReadModel') && projectUiSource.includes('eventLedgerReadModel') && projectUiSource.includes('backend-sync-timeline-events') && projectUiSource.includes('Sync Timeline'), 'Project dashboard must consume backend timeline and event-ledger read models when online instead of treating browser project logs as the only workflow source.');
assert(projectUiSource.includes('Backend Worker Station') && projectUiSource.includes('/workers/autonomous/status') && projectUiSource.includes('/workers/autonomous/${action}') && projectUiSource.includes("runBackendSchedulerAction('start')") && projectUiSource.includes("runBackendSchedulerAction('stop')") && projectUiSource.includes('backend-scheduler-agent-controls') && projectUiSource.includes('AGENT CONTROL') && projectUiSource.includes('STRATEGY'), 'Project dashboard must expose backend autonomous scheduler status and Agent autonomy controls.');
assert(projectUiSource.includes('Agent Runs') && projectUiSource.includes('agentProcessedCount') && projectUiSource.includes('Agent Skips'), 'Backend Worker Station must surface per-Agent scheduler counters.');
assert(projectUiSource.includes('runImmediately: true') && projectUiSource.includes('includeReadModels: false') && projectUiSource.includes('manager-ui-scheduler-start-pulse') && projectUiSource.includes('Starting backend scheduler') && projectUiSource.includes('startPending: true') && projectUiSource.includes('startPending: false') && projectUiSource.includes('lastStartedRunImmediately: false') && !projectUiSource.includes('backend-scheduler-start-first-work') && !projectUiSource.includes('persistActiveProject') && projectUiSource.includes('silent = false') && projectUiSource.includes('Immediate Start') && projectUiSource.includes('Latest Backend Work') && projectUiSource.includes('backend-last-result'), 'Backend Worker Station start must immediately kick current-project and Agent startup work through lightweight receipts, refresh silently, avoid optimistic browser-authored worker results, and surface the latest processed project/Agent result only after backend proof returns.');
assert(appSource.includes('projectHasBackendSyncEvidence') && appSource.includes('hasBackendManagedProjectMarker') && appSource.includes('!hasBackendManagedProjectMarker(project)') && appSource.includes('!allowLocalRuntimeFallbackForActiveProject(project)') && appSource.includes('backendStation.projectCatalog') && appSource.includes('backendStation.lastManagerDashboardSyncAt') && appSource.includes('Local autonomous cycle blocked; backend route required') && appSource.includes('Local fallback disabled for backend-synced project.'), 'Browser-local autonomous scheduler and direct local autonomous cycle calls must be disabled for backend-synced or backend-marked real projects even when the local station status is stale; local cycles are allowed only for offline-only, demo/sample, or explicit development fallback projects.');
...
assert(projectUiSource.includes('manager-ui-backend-pulse') && projectUiSource.includes('Server Pulse'), 'Project dashboard must expose a backend-backed project pulse path.');
...
assert(projectUiSource.includes('Sync State') && projectUiSource.includes('syncBackendProjectState') && projectUiSource.includes('applyBackendProjectSnapshot') && projectUiSource.includes('saveActiveProjectToBackend') && projectUiSource.includes('backend-save-project') && projectUiSource.includes('Seed Sample/Dev') && projectUiSource.includes('canUseDevelopmentSnapshotSeed') && projectUiSource.includes('!hasBackendManagedProjectMarker(project)') && projectUiSource.includes('Browser snapshot seeding is disabled for real backend projects.') && projectUiSource.includes('seedDisabled: backendStation.loading || !canSeedActiveProjectSnapshotToBackend(activeProject)') && projectUiSource.includes('disabled={seedDisabled}'), 'Project dashboard must sync backend project snapshots back into the manager UI and limit browser snapshot seeding to sample/dev fallback projects.');
...
assert(projectUiSource.includes('syncBackendProjectCatalog') && projectUiSource.includes('/projects') && projectUiSource.includes('projectCatalog') && projectUiSource.includes('backend-sync-project-catalog') && projectUiSource.includes('Sync Projects'), 'Project dashboard must list/load backend projects from the backend catalog instead of treating localStorage as the only project source.');
assert(appSource.includes('readStoredProjectArray') && appSource.includes('cachedBrowserProjectIds') && appSource.includes('if (!project) return false;') && appSource.includes('isBackendManagedRealProject') && appSource.includes('canPersistProjectToBrowserCache') && appSource.includes('canPersistChatMessageToBrowserCache') && appSource.includes('const browserCacheProjects = projects.filter(canPersistProjectToBrowserCache)') && appSource.includes('const browserCacheMessages = chatMessages') && appSource.includes('isUnscopedProofLikeChatMessage') && appSource.includes('!isManagerDemoProject(project)'), 'Browser localStorage project/chat cache must gate persisted projects and chat messages through explicit cache policies that exclude demo fixtures and orphan proof-like messages.');
assert(projectUiSource.includes('syncBackendManagerDashboard') && projectUiSource.includes('/manager-dashboard') && projectUiSource.includes('Backend Manager Snapshot') && projectUiSource.includes('backend-manager-dashboard-snapshot'), 'Project dashboard must pull and show the backend manager-dashboard aggregate snapshot.');
assert(projectUiSource.includes('Manager dashboard sync') && projectUiSource.includes('Proof Routes') && projectUiSource.includes('Ops Agents') && projectUiSource.includes('Management Checks') && projectUiSource.includes('Assignment Rows') && projectUiSource.includes('Change Rows'), 'Backend manager-dashboard snapshot must expose readiness proof, operations, management, assignment, and change counts.');
assert(projectUiSource.includes('syncBackendManagerCommandCenter') && projectUiSource.includes('/manager-command-center') && projectUiSource.includes('Command center sync') && projectUiSource.includes('Sync Command') && projectUiSource.includes('backend-sync-command-center') && projectUiSource.includes('backend-manager-command-center-snapshot'), 'Project dashboard must sync and show the standalone manager command center endpoint without pre-writing stale browser snapshots.');
assert(projectUiSource.includes('syncBackendManagerScenarioWalkthrough') && projectUiSource.includes('/manager-scenario-walkthrough') && projectUiSource.includes('Scenario walkthrough sync') && projectUiSource.includes('Sync Walkthrough') && projectUiSource.includes('backend-sync-scenario-walkthrough') && projectUiSource.includes('backend-manager-scenario-walkthrough-snapshot'), 'Project dashboard must sync and show the standalone manager scenario walkthrough endpoint without pre-writing stale browser snapshots.');
assert(projectUiSource.includes('syncBackendManagerScenarioTrail') && projectUiSource.includes('/manager-scenario-trail') && projectUiSource.includes('Scenario trail sync') && projectUiSource.includes('Standalone Trail') && projectUiSource.includes('backend-sync-scenario-trail') && projectUiSource.includes('backend-manager-scenario-trail-snapshot'), 'Project dashboard must sync and show the standalone manager scenario trail endpoint without pre-writing stale browser snapshots.');
assert(projectUiSource.includes('syncBackendManagerRequirementMatrix') && projectUiSource.includes('/manager-requirement-matrix') && projectUiSource.includes('Requirement matrix sync') && projectUiSource.includes('Sync Matrix') && projectUiSource.includes('backend-sync-requirement-matrix') && projectUiSource.includes('backend-manager-requirement-matrix-snapshot'), 'Project dashboard must sync and show the standalone manager requirement matrix endpoint without pre-writing stale browser snapshots.');
assert(projectUiSource.includes('syncBackendSyncProtocolAudit') && projectUiSource.includes('/sync-protocol-audit') && projectUiSource.includes('Sync protocol audit sync') && projectUiSource.includes('Sync Protocol') && projectUiSource.includes('backend-sync-sync-protocol-audit') && projectUiSource.includes('backend-sync-protocol-audit-snapshot'), 'Project dashboard must sync and show the standalone sync protocol audit endpoint without pre-writing stale browser snapshots.');
assert(projectUiSource.includes('syncBackendManagerUseCaseAudit') && projectUiSource.includes('/manager-use-case-audit') && projectUiSource.includes('Use case audit sync') && projectUiSource.includes('Sync Audit') && projectUiSource.includes('backend-sync-use-case-audit') && projectUiSource.includes('backend-manager-use-case-audit-snapshot'), 'Project dashboard must sync and show the standalone manager use case audit endpoint without pre-writing stale browser snapshots.');
assert(projectUiSource.includes('syncBackendManagerActionQueue') && projectUiSource.includes('/manager-action-queue') && projectUiSource.includes('Action queue sync') && projectUiSource.includes('Sync Queue') && projectUiSource.includes('backend-sync-action-queue') && projectUiSource.includes('backend-manager-action-queue-snapshot'), 'Project dashboard must sync and show the standalone manager action queue endpoint without pre-writing stale browser snapshots.');
assert(projectUiSource.includes('syncBackendAgentAutonomousActionQueue') && projectUiSource.includes('/agent-autonomous-action-queue') && projectUiSource.includes('Agent autonomous queue sync') && projectUiSource.includes('Sync Agent Queue') && projectUiSource.includes('backend-sync-agent-autonomous-action-queue') && projectUiSource.includes('runAgentAutonomousActionQueueRow') && projectUiSource.includes('onRunRow: runAgentAutonomousActionQueueRow') && agentAutonomousActionQueueUiSource.includes('backend-agent-autonomous-action-queue-snapshot') && agentAutonomousActionQueueUiSource.includes('backend-agent-autonomous-action-run-'), 'Project dashboard must sync, show, and run the standalone Agent autonomous action queue endpoint without local mock mutation.');
assert(projectUiSource.includes('syncBackendManagerReadyPackage') && projectUiSource.includes('/manager-ready-package') && projectUiSource.includes('Ready package sync') && projectUiSource.includes('Manager Ready Package') && projectUiSource.includes('backend-sync-ready-package') && projectUiSource.includes('backend-manager-ready-package-snapshot'), 'Project dashboard must sync and show the manager ready package endpoint without pre-writing stale browser snapshots.');
assert(projectUiSource.includes('syncBackendReadyPackageSubmodels') && projectUiSource.includes('/brainstorm-layer') && projectUiSource.includes('/artifact-quality-audit') && projectUiSource.includes('/submission-review-workflow') && projectUiSource.includes('/product-team-delivery-trace') && projectUiSource.includes('missingProductTeamDeliveryTrace') && projectUiSource.includes('fetch-product-team-delivery-trace') && projectUiSource.includes('/product-team-operating-loop') && projectUiSource.includes('missingProductTeamOperatingLoop') && projectUiSource.includes('fetch-product-team-operating-loop') && projectUiSource.includes('/evidence-quality-audit') && projectUiSource.includes('/evidence-source-review-workflow') && projectUiSource.includes('/evidence-custody-readiness') && projectUiSource.includes('readyPackageSubmodels') && projectUiSource.includes('backend-sync-proof-models') && projectUiSource.includes('Sync Proof Models'), 'Manager Ready Package proof subpanels must refresh standalone backend read models and expose backend-required missing state for delivery trace and operating loop instead of depending only on embedded package snapshots or frontend summary fallbacks.');
assert(serviceSource.includes('productTeamAcceptanceChainRoutes') && serviceSource.includes('readyForGenericProductTeamAcceptance') && serviceSource.includes('readyForBsideProductTeamRun') && projectUiSource.includes('proof-map-product-team-acceptance-chain') && projectUiSource.includes('Generic Product-Team Acceptance Chain'), 'Readiness Proof Map and Manager UI must expose the generic product-team acceptance chain as a route-backed C/A acceptance proof instead of scattered mock panels.');
assert(
  appSource.includes('missingBrainstormLayer')
    ...
    && projectUiSource.includes('backend-brainstorm-layer-source')
    && projectUiSource.includes('backend-artifact-quality-audit-source')
    && projectUiSource.includes('backend-submission-review-workflow-source')
    && projectUiSource.includes('backend-evidence-quality-audit-source')
    && projectUiSource.includes('backend-evidence-source-review-workflow-source')
    && projectUiSource.includes('backend-evidence-custody-readiness-source')
...
assert(
  appSource.includes('refreshBackendManagerView')
    && appSource.includes('onSyncManagerView: refreshBackendManagerView')
    && projectUiSource.includes('data-testid="backend-sync-manager-view"')
    && projectUiSource.includes('onClick={onSyncManagerView}')
    && projectUiSource.includes('Sync Manager View'),
  'Backend Worker Station must let managers manually refresh only the aggregate manager-dashboard view.',
);
...
assert(projectUiSource.includes('backend-url-input') && projectUiSource.includes('Save URL') && appSource.includes('hall_of_fame_studio.agent_backend_url.v1'), 'Backend Worker Station must let managers configure and persist the backend URL.');
...
assert(
  appSource.includes("action: 'submission-review-failed'")
    && appSource.includes('No local review receipt was created.')
    && projectUiSource.includes('Review write failed:')
    && projectUiSource.includes("reviewReceipt?.action === 'submission-review-failed'")
    && projectUiSource.includes('no local review receipt was created.'),
  'Reviewer composer backend failures must replace pending receipts with a visible failed state and must not create local review proof.',
);
...
assert(appSource.includes('startAutonomousRunControlSession') && appSource.includes('tickAutonomousRunControlSession') && appSource.includes('pauseAutonomousRunControlSession') && appSource.includes('cancelAutonomousRunControlSession') && projectUiSource.includes('backend-autonomous-run-control-session-start') && projectUiSource.includes('backend-autonomous-run-control-session-tick-receipt') && projectUiSource.includes('backend-autonomous-run-control-session-cancel') && projectUiSource.includes('Autopilot session:') && projectUiSource.includes('targetReadyCount'), 'React Backend Worker Station must let the manager start, tick, pause, cancel, and inspect an autonomous run-control session with delivery target progress.');
assert(appSource.includes('runAutopilotSessionThroughScheduler') && appSource.includes('ensureBackendAutopilotSessionForScheduler') && appSource.includes('extractAutopilotProviderEvidenceReceipt') && appSource.includes("requestAgentBackend('/workers/autopilot/due'") && appSource.includes('providerEvidenceSearchEnabled: true') && projectUiSource.includes('backend-autonomous-run-control-session-scheduler-tick') && projectUiSource.includes('backend-autonomous-run-control-session-worker-receipt') && projectUiSource.includes('backend-autopilot-provider-evidence-receipt') && projectUiSource.includes('Provider evidence:'), 'React Backend Worker Station must confirm a durable backend Autopilot session, advance it through the dedicated due-worker lane, request provider evidence, and show the Autopilot scheduler/provider evidence receipt.');
assert(timelineUiSource.includes('chatProofIdsFromAttachment') && timelineUiSource.includes('providerEvidenceMessageId') && timelineUiSource.includes('providerEvidenceTranscriptRoute') && timelineUiSource.includes('flow-open-transcript-') && timelineUiSource.includes('Transcript proof'), 'Manager Flow Graph attachments must expose provider-evidence transcript proof jumps into backend Group Chat.');
...
assert(
  appSource.includes('runBackendAgentMessage')
    && appSource.includes('/message')
    && appSource.includes('runBackendAgentMessage,')
    && projectUiSource.includes('Agent Message')
    && projectUiSource.includes('agent-message-send-')
    && projectUiSource.includes('onClick={() => runBackendAgentMessage(agent.id)}'),
  'Project dashboard must expose backend-backed Agent-to-Agent message publishing controls.',
);
assert(agentAutonomousActionQueueUiSource.includes('Agent Action Failed') && agentAutonomousActionQueueUiSource.includes('No local run receipt was created:') && agentAutonomousActionQueueUiSource.includes('backend-agent-autonomous-action-run-output-failed') && appSource.includes('agentAutonomousActionRun: null') && appSource.includes('localProofCreated: false'), 'Failed Agent Autonomous Action Queue runs must clear stale run receipts and show a non-proof failed state.');
assert(projectUiSource.includes('Intent Run Failed') && projectUiSource.includes('No local intent receipt was created.') && projectUiSource.includes('backend-collaboration-intent-run-output-failed') && appSource.includes('collaborationIntentRun: null') && appSource.includes('localProofCreated: false'), 'Failed Collaboration Intent Queue runs must clear stale delegated receipts and show a non-proof failed state.');
assert(projectUiSource.includes('Action failed:') && projectUiSource.includes('No local operator receipt was created.') && appSource.includes('mvpReadinessOperatorActionRun') && appSource.includes('localProofCreated: false'), 'Failed MVP readiness operator actions must show failure instead of leaving a stale operator receipt.');
assert(projectUiSource.includes('Agent Communication Flow') && projectUiSource.includes('agentCommunicationRows') && projectUiSource.includes('Sender Worklog') && projectUiSource.includes('Agent chat proof'), 'Project dashboard must expose Agent-to-Agent communication flow proof.');
assert(projectUiSource.includes('agent-message-delivery-matrix') && projectUiSource.includes('Agent Message Delivery Matrix') && projectUiSource.includes('Direct Receipt') && projectUiSource.includes('Target Inbox') && projectUiSource.includes('Delivery chat proof') && projectUiSource.includes('Delivered Messages'), 'Project dashboard must expose per-target Agent message delivery from group chat receipt to target inbox.');
assert(projectUiSource.includes('proof-map-agent-message-routes') && projectUiSource.includes('backendAgentMessageSummary') && projectUiSource.includes('Agent-to-Agent message routes') && projectUiSource.includes('Agent message timeline proof'), 'Manager Proof Map must render backend Agent-to-Agent message routes with timeline proof exits.');
assert(projectUiSource.includes('agent-priority-') && projectUiSource.includes('latestAgentWorker.managementPriority') && projectUiSource.includes('priorityReasons.slice'), 'Project dashboard must expose visible Agent worker priority and reasons in each Team row.');
assert(projectUiSource.includes('agent-state-detail-') && projectUiSource.includes('Latest Inbox') && projectUiSource.includes('Open Obligation') && projectUiSource.includes('Latest Worklog') && projectUiSource.includes('Next Agent Run'), 'Project dashboard must expose each Agent as an independent visible state surface.');
assert(projectUiSource.includes('selectedAgentFocusId') && projectUiSource.includes('agent-focus-open-') && projectUiSource.includes('agent-focus-panel-') && projectUiSource.includes('Agent Focus Workspace') && projectUiSource.includes('Owned Task Evidence') && projectUiSource.includes('agent-focus-chat-proof-') && projectUiSource.includes('agent-focus-timeline-proof-') && projectUiSource.includes('Management Surface') && projectUiSource.includes('agent-focus-management-proof-'), 'Project dashboard must let managers open a dedicated per-Agent focus workspace with plan, inbox, obligations, owned tasks, management surface, and proof routes.');
assert(projectUiSource.includes('agentDashboardSnapshots') && projectUiSource.includes('agentDashboardSnapshotKey') && projectUiSource.includes('agentDashboardSnapshotFor') && projectUiSource.includes('syncBackendAgentDashboard') && projectUiSource.includes('/agents/${encodeURIComponent(agentId)}/dashboard') && projectUiSource.includes('Backend Agent Dashboard') && projectUiSource.includes('agent-focus-backend-dashboard-') && projectUiSource.includes('agent-focus-backend-cadence-') && projectUiSource.includes('agent-focus-backend-control-runs-') && projectUiSource.includes('agent-focus-control-run-receipt-') && projectUiSource.includes('agent-focus-dashboard-source-') && projectUiSource.includes('agent-focus-backend-dashboard-required-') && projectUiSource.includes('Sync Agent Dashboard') && projectUiSource.includes('local Agent state stays visible as project snapshot context') && projectUiSource.includes('Next Run') && projectUiSource.includes('Management Priority') && projectUiSource.includes('Routine') && projectUiSource.includes('Control Runs'), 'Per-Agent focus workspace must sync and display the backend Agent dashboard read model, scope cached snapshots by project plus Agent, show backend-required missing state for real projects, and expose cadence, management priority, fixed routine, and autonomous control run receipts when online.');
...
assert(projectUiSource.includes('agent-focus-pulse-') && projectUiSource.includes('Run Agent Pulse') && projectUiSource.includes('runBackendAgentPulse(agent.id)'), 'Per-Agent focus workspace must let managers trigger that Agent fixed work pulse directly.');
assert(projectUiSource.includes('agent-inbox-proof-') && projectUiSource.includes('agent-obligation-proof-') && projectUiSource.includes('agent-worklog-timeline-') && projectUiSource.includes('openProjectChatProof') && projectUiSource.includes('openProjectTimelineProof'), 'Project dashboard must let managers jump from each Agent state surface to exact chat and timeline proof.');
assert(projectUiSource.includes('Leader Assignment Flow') && projectUiSource.includes('Group @Assignment') && projectUiSource.includes('Assignee Inbox') && projectUiSource.includes('Acknowledgement') && projectUiSource.includes('Work Pulse') && projectUiSource.includes('Assignment chat proof') && projectUiSource.includes('Assignment timeline proof'), 'Project dashboard must expose a manager-readable Leader @assignment flow from group chat to Agent inbox, acknowledgement, work pulse, and timeline proof.');
assert(projectUiSource.includes('assignmentFlowRows') && projectUiSource.includes('ownerState.inbox') && projectUiSource.includes('ownerState.obligations') && projectUiSource.includes('ownerState.worklog'), 'Leader Assignment Flow must derive visibility from the assigned Agent state, not only from task metadata.');
assert(projectUiSource.includes('assignment-timeline-matrix') && projectUiSource.includes('Assignment Timeline Matrix') && projectUiSource.includes('@Assignment Posted') && projectUiSource.includes('Assignee Saw It') && projectUiSource.includes('Assignment Timeline Event') && projectUiSource.includes('Assignment receipt proof') && projectUiSource.includes('Assignment timeline event proof'), 'Project dashboard must expose a Leader assignment timeline matrix from group @mention to assignee receipt and timeline event proof.');
assert(projectUiSource.includes('assignmentTimelineMatrixRows') && projectUiSource.includes('assignmentTimelineIds') && projectUiSource.includes('workTimelineIds') && projectUiSource.includes('Assignment Timeline'), 'Leader assignment timeline matrix must derive assignment, acknowledgement, and work-pulse timeline proof rows.');
...
assert(projectUiSource.includes('assignment-work-progress-matrix') && projectUiSource.includes('Assignment Work Progress Matrix') && projectUiSource.includes('Progress Chat') && projectUiSource.includes('Timeline Progress') && projectUiSource.includes('Completion Proof') && projectUiSource.includes('Progress timeline proof') && projectUiSource.includes('Completion timeline proof') && projectUiSource.includes('Assignment Progress'), 'Project dashboard must expose assigned-work progress, latest update, and completion proof mapped to the timeline.');
...
assert(projectUiSource.includes('managerAssignmentDraft') && projectUiSource.includes('projectManagerUiStateKey') && projectUiSource.includes('managerAssignmentDraftFor') && projectUiSource.includes('updateManagerAssignmentDraft') && projectUiSource.includes('Leader Assignment Composer') && projectUiSource.includes('manager-leader-assignment-composer') && projectUiSource.includes('manager-assignment-composer-input') && projectUiSource.includes('manager-assignment-composer-target') && projectUiSource.includes('manager-assignment-composer-submit') && projectUiSource.includes('submitManagerLeaderAssignment') && !projectUiSource.includes('setManagerAssignmentDraft(prev'), 'Project dashboard must let managers type custom work for the confirmed Leader to @assign in group chat, with the draft scoped to the active project.');
assert(projectUiSource.includes('Manager Proof Map') && projectUiSource.includes('manager-proof-map') && projectUiSource.includes('managerProofMapDisplayRows') && projectUiSource.includes("schemaVersion: 'manager-proof-map/v1'") && projectUiSource.includes("missingBackendReadModel('manager-proof-map/v1'") && projectUiSource.includes('manager-proof-map-source') && projectUiSource.includes('openManagerProofMapRow') && projectUiSource.includes('Every readiness condition has a direct evidence route') && projectUiSource.includes('transcriptProofCoverageSummary') && projectUiSource.includes('proof-map-transcript-proof-coverage') && projectUiSource.includes('Transcript coverage proof'), 'Manager Scenario Readiness must expose an actionable backend-first proof map for every manager-ready condition, including backend transcript proof coverage.');
assert(projectUiSource.includes('proof-map-') && projectUiSource.includes('Kickoff chat proof') && projectUiSource.includes('Group chat proof') && projectUiSource.includes('Timeline proof') && projectUiSource.includes('Change proof') && projectUiSource.includes('Management proof'), 'Manager Proof Map must route checks to kickoff, group chat, timeline, change, and management evidence.');
assert(projectUiSource.includes('Scenario Control Center') && projectUiSource.includes('scenario-control-center') && projectUiSource.includes('Kickoff Decisions') && projectUiSource.includes('24/7 Work Pulse') && projectUiSource.includes('Agent Management Sync') && projectUiSource.includes('Mid-project Change Intake') && projectUiSource.includes('Manager Evidence Exit') && projectUiSource.includes('scenario-control-action-'), 'Project dashboard must expose a manager-first scenario control center from kickoff decisions through 24/7 work, management sync, change intake, and proof exit.');
...
assert(
  appSource.includes('const backendCommandAvailable = shouldAttemptBackendProjectWrite(activeProject)')
    && appSource.includes('(backendCommandAvailable || isManagerDemoProject(activeProject))')
    && !projectUiSource.includes('(backendOnline || isManagerDemoProject(activeProject))')
    && appSource.includes('if (!activeProject || !backendCommandAvailable || !managerCommandCenter.nextBestAction?.canRun) return null;')
    && appSource.includes('if (!activeProject || !backendCommandAvailable || !row || !row.primaryAction?.canRun) return null;')
    && appSource.includes('runManagerCommandCenterNext,')
    && appSource.includes('onRunRow: runManagerScenarioWalkthroughRow')
    && appSource.includes('onRunRow: runManagerActionPlaybookRow')
    && projectUiSource.includes('disabled={!backendCommandAvailable || backendStation.loading || !managerCommandCenter.nextBestAction?.canRun}')
    && projectUiSource.includes('disabled={!backendCommandAvailable || backendStation.loading || !row.primaryAction?.canRun || row.primaryAction?.routeResolved === false')
    && projectUiSource.includes('disabled={!backendCommandAvailable || backendStation.loading || !row.canRun || row.routeResolved === false'),
  'Manager command, walkthrough, and action queue controls must remain visible and backend-attemptable based on configured project/backend command availability, not a stale frontend online status label.',
);
assert(appSource.includes('if (!activeProject || !backendCommandAvailable || !action || !action.canRun) return null;') && appSource.includes('if (!activeProject || !backendCommandAvailable) return null;') && appSource.includes('commandDisabled: !backendCommandAvailable || backendStation.loading') && projectUiSource.includes('disabled={commandDisabled || !action.canRun}') && projectUiSource.includes('runDisabled: !backendCommandAvailable || backendStation.loading') && agentAutonomousActionQueueUiSource.includes('disabled={runDisabled || Boolean(effectivePendingAgentId) || !row.canRun || row.routeResolved === false}'), 'Autonomous Run Control and Agent Autonomous Queue actions must attempt backend routes whenever the project has a backend command target, even if the station status label is stale.');
...
assert(projectUiSource.includes('backend-provider-eval-record-shadow-replay') && projectUiSource.includes('manager-ui-provider-eval-receipt') && projectUiSource.includes("workflowKey: 'providerEvalRunWorkflow'") && projectUiSource.includes("receiptKey: 'providerEvalRun'"), 'Manager UI must let C-side operators record provider eval shadow replay through the backend receipt path.');
assert(projectUiSource.includes('runBackendProductionControlReceipt') && projectUiSource.includes('manager-ui-production-control-receipt') && projectUiSource.includes("evidenceEnvironment: 'local-rehearsal'") && projectUiSource.includes('backend-production-operations-record-controls') && projectUiSource.includes('backend-production-deployment-record-controls') && projectUiSource.includes('backend-production-security-record-controls') && projectUiSource.includes('backend-production-provider-record-controls'), 'Manager UI production-control buttons must write backend local-rehearsal receipts instead of frontend state or public-production certification.');
assert(
  projectUiSource.includes('backend-private-pilot-record-release-candidate')
    && projectUiSource.includes('backend-private-pilot-record-launch-run')
    && projectUiSource.includes('backend-private-pilot-record-launch-health')
    && projectUiSource.includes('backend-private-pilot-record-acceptance-report')
    && projectUiSource.includes('backend-launch-approval-record-manager')
    && projectUiSource.includes('backend-launch-approval-record-security')
    && projectUiSource.includes('backend-project-evidence-export-request')
    && projectUiSource.includes('backend-project-evidence-export-approve-manager')
    && projectUiSource.includes('backend-project-evidence-export-approve-security')
    && projectUiSource.includes('backend-project-evidence-export-record-download-audit')
    && projectUiSource.includes('privatePilotReleaseCandidateWorkflow')
    && projectUiSource.includes('privatePilotLaunchRunWorkflow')
    && projectUiSource.includes('privatePilotLaunchHealthCheckWorkflow')
    && projectUiSource.includes('privatePilotAcceptanceReportWorkflow'),
...
assert(projectDashboardManagerScenarioWalkthroughSource.includes('Manager Scenario Walkthrough') && projectDashboardManagerScenarioWalkthroughSource.includes('manager-scenario-walkthrough') && projectDashboardManagerScenarioWalkthroughSource.includes('Run walkthrough step') && projectDashboardManagerScenarioWalkthroughSource.includes('Walkthrough proof') && projectDashboardManagerScenarioWalkthroughSource.includes('Primary action:') && projectDashboardManagerScenarioWalkthroughSource.includes('Result inspection:') && projectDashboardManagerScenarioWalkthroughSource.includes('Run result proof') && projectDashboardManagerScenarioWalkthroughSource.includes('manager-walkthrough-run-') && appSource.includes('runManagerScenarioWalkthroughRow') && appSource.includes("/manager-scenario-walkthrough/${encodeURIComponent(row.id || 'next')}/run") && projectUiSource.includes('Walkthrough route:'), 'Project dashboard must expose a guided manager scenario walkthrough that links each story stage to runnable actions, result inspection, and proof exits.');
assert(projectDashboardManagerActionPlaybookSource.includes('manager-action-playbook') && projectDashboardManagerActionPlaybookSource.includes('Manager Action Playbook') && projectDashboardManagerActionPlaybookSource.includes('Operational next steps mapped to runnable backend routes and exact proof exits') && appSource.includes('managerActionPlaybook') && projectDashboardManagerActionPlaybookSource.includes('manager-action-playbook-open-') && projectDashboardManagerActionPlaybookSource.includes('manager-action-playbook-run-') && projectDashboardManagerActionPlaybookSource.includes('Open Step') && projectDashboardManagerActionPlaybookSource.includes('Run Action') && projectDashboardManagerActionPlaybookSource.includes('Run Again') && projectDashboardManagerActionPlaybookSource.includes('rerunnable'), 'Project dashboard must expose a manager-readable action playbook with backend routes, executable/rerunnable actions, and proof exits.');
assert(projectUiSource.includes('manager-scenario-trail') && projectDashboardManagerScenarioTrailSource.includes('Manager Scenario Trail') && projectUiSource.includes('Project Brief Heard') && projectUiSource.includes('Leader Marker Confirmed') && projectUiSource.includes('Assigned Work Progress') && projectUiSource.includes('Meeting + Google Chat Change') && projectDashboardManagerScenarioTrailSource.includes('Trail proof') && projectUiSource.includes('Scenario Trail'), 'Project dashboard must expose a single manager-readable end-to-end scenario trail with proof jumps.');
...
assert(projectUiSource.includes('sync-protocol-audit') && projectDashboardSyncProtocolAuditSource.includes('Sync Protocol Audit') && projectDashboardSyncProtocolAuditSource.includes('Backend collaboration protocol') && projectDashboardSyncProtocolAuditSource.includes('Protocol timeline proof') && projectDashboardSyncProtocolAuditSource.includes('Agent State') && projectDashboardSyncProtocolAuditSource.includes('Ledger'), 'Project dashboard must expose a manager-readable backend sync protocol audit with proof jumps.');
...
assert(projectUiSource.includes('backendOrAllowedFallback') && projectUiSource.includes('frontendMockSuppressed') && projectUiSource.includes('backend model missing') && projectUiSource.includes('demo data') && projectUiSource.includes('backend-backed'), 'Project dashboard must label Manager read-model provenance and suppress frontend fallback rows when backend read models are required.');
assert(projectUiSource.includes('managerReadModelSourceLabel') && projectUiSource.includes('managerReadModelSourceClass') && projectUiSource.includes('manager-command-center-source') && projectDashboardManagerScenarioWalkthroughSource.includes('manager-scenario-walkthrough-source') && projectDashboardManagerActionPlaybookSource.includes('manager-action-playbook-source') && projectDashboardManagerScenarioTrailSource.includes('manager-scenario-trail-source') && projectUiSource.includes('manager-use-case-audit-source') && projectDashboardSyncProtocolAuditSource.includes('sync-protocol-audit-source'), 'Manager governance surfaces must expose visible backend-backed/demo/backend-required source labels for C-side proof controls.');
assert(projectUiSource.includes('manager-flow-graph/missing-backend') && projectUiSource.includes('manager-flow-source-label') && projectUiSource.includes('manager-flow-backend-required') && projectUiSource.includes('backendFlowGraphReady') && projectUiSource.includes('const allowFlowFrontendFallbacks = allowLocalRuntimeFallbackForActiveProject(activeProject)') && projectUiSource.includes('onClick={() => syncBackendManagerFlowGraph({ silent: false })}') && projectUiSource.includes('disabled={!backendCommandAvailable || backendStation.loading}') && projectUiSource.includes('onClick={() => confirmManagerFlowNode(selectedNode.id, true)}') && projectUiSource.includes('onClick={() => confirmManagerFlowNode(selectedNode.id, false)}'), 'Manager Flow Graph must suppress frontend fallback rows for backend-synced real projects and keep Sync/Confirm/Supersede backend-attemptable from the configured command target rather than a stale frontend online status label.');
assert(projectUiSource.includes('agent-workbench-') && projectUiSource.includes('runBackendAgentEvidenceSearch') && projectUiSource.includes('runBackendAgentArtifactSubmission') && projectUiSource.includes('runBackendAgentArtifactDraft'), 'Agent Focus Workspace must expose a backend-backed Agent Workbench for evidence, artifact submissions, and draft submissions.');
...
assert(projectUiSource.includes('agentWorkbenchFailurePatch') && projectUiSource.includes('Agent Workbench Write Failed') && projectUiSource.includes('provider-evidence-search-failed') && projectUiSource.includes('manual-source-record-failed') && projectUiSource.includes('artifact-submission-failed') && projectUiSource.includes('artifact-draft-submit-failed') && projectUiSource.includes('no local workbench proof was created') && projectUiSource.includes('localProofCreated: false'), 'Agent Workbench failed evidence/submission/draft writes must replace stale receipts with visible non-proof failure state.');
...
assert(
  projectUiSource.includes('if (!activeProject || !agentId || !shouldAttemptBackendProjectWrite(activeProject)) return;')
    && projectUiSource.includes('if (!activeProject?.id || !submission?.id || !shouldAttemptBackendProjectWrite(activeProject)) return;')
    && projectUiSource.includes('const workbenchWriteDisabled = !backendCommandAvailable || backendStation.loading || workbenchBackendContextMissing;')
    && projectUiSource.includes('disabled={workbenchWriteDisabled || workbenchTaskOptions.length === 0}')
    && projectUiSource.includes('disabled={workbenchWriteDisabled || workbenchReviewOptions.length === 0}')
    && projectUiSource.includes('disabled={workbenchWriteDisabled}')
    && projectUiSource.includes('disabled={!backendCommandAvailable || backendStation.loading}')
    && appSource.includes('reviewSubmitDisabled: (reviewerId) => !backendCommandAvailable || backendStation.loading || !reviewerId')
    && projectUiSource.includes('disabled={reviewSubmitDisabled(rowReviewerId)}')
    && projectUiSource.includes('disabled={!backendCommandAvailable || backendStation.loading || !selectedMessageTarget}'),
  'Agent Workbench, Reviewer composer, Agent Pulse, and Agent-to-Agent message controls must remain backend-attemptable from the configured backend command target while Workbench also fails closed when a backend-required Agent Dashboard is missing.',
);
assert(projectUiSource.includes('AGENT_WORKBENCH_ARTIFACT_TYPES') && projectUiSource.includes('discovery-report') && projectUiSource.includes('brainstorm-board') && projectUiSource.includes('evidence-packet') && projectUiSource.includes('product-brief') && projectUiSource.includes('decision-proposal') && projectUiSource.includes('risk-review') && projectUiSource.includes('implementation-plan') && projectUiSource.includes('final-deliverable'), 'Agent Workbench must stay generic to product-team artifact nodes rather than becoming a research-only surface.');
...
assert(projectUiSource.includes('manager-live-command-center') && projectUiSource.includes('Manager Live Command Center') && projectUiSource.includes('Next best action:') && projectUiSource.includes('Kickoff Decision Board') && projectUiSource.includes('Leader Marker') && projectUiSource.includes('Next Actions Confirmed') && projectUiSource.includes('Work Loop Board') && projectUiSource.includes('Loop proof') && projectUiSource.includes('Collaboration Board') && projectUiSource.includes('Leader @assignments') && projectUiSource.includes('Agent messages') && projectUiSource.includes('Collaboration proof') && projectUiSource.includes('Change Protocol Board') && projectUiSource.includes('Owner Plan Updated') && projectUiSource.includes('Team Resync') && projectUiSource.includes('Change protocol proof') && projectUiSource.includes('Attention Queue') && projectUiSource.includes('Agent Readiness') && projectUiSource.includes('Latest @Signal') && projectUiSource.includes('Work Started') && projectUiSource.includes('Change Owner Sync') && projectUiSource.includes('Owner Confirmed') && projectUiSource.includes('Plan Updated') && projectUiSource.includes('manager-command-kickoff-proof-') && projectUiSource.includes('manager-command-work-loop-proof-') && projectUiSource.includes('manager-command-collaboration-proof-') && projectUiSource.includes('manager-command-change-protocol-proof-') && projectUiSource.includes('manager-command-change-proof-') && projectUiSource.includes('manager-command-agent-inbox-proof-') && projectUiSource.includes('manager-command-run-next') && projectUiSource.includes('runManagerCommandCenterNext') && projectUiSource.includes('manager-command-run-receipt') && projectUiSource.includes('Command run proof'), 'Project dashboard must expose a manager live command center with kickoff decisions, 24/7 work-loop proof, collaboration proof, change protocol proof, next action, attention queue, Agent receipt readiness, change owner sync, and run receipts.');
...
assert(projectUiSource.includes('manager-requirement-matrix') && projectUiSource.includes('Manager Requirement Matrix') && projectUiSource.includes('Each requested condition mapped to concrete chat, timeline, or read-model proof') && projectUiSource.includes('Requirement proof') && projectUiSource.includes('Requirements'), 'Project dashboard must expose a manager requirement matrix that maps requested conditions to evidence.');
...
assert(projectUiSource.includes('Manager Use Case Audit') && projectUiSource.includes('manager-use-case-audit') && projectUiSource.includes('Use case proof') && projectUiSource.includes('Run use case action') && projectUiSource.includes('Next action:') && projectUiSource.includes('manager-use-case-run-') && projectUiSource.includes('Kickoff Meeting') && projectUiSource.includes('Group @Assignment') && projectUiSource.includes('Owner Confirmation'), 'Project dashboard must expose a manager-readable use case audit that groups the requested story into proof-backed stages and runnable next actions.');
...
assert(projectDashboardManagerActionPlaybookSource.includes('Body template') && appSource.includes('Next body:') && projectDashboardManagerActionPlaybookSource.includes('Run route:') && projectDashboardManagerActionPlaybookSource.includes('requestBodyTemplate') && projectDashboardManagerActionPlaybookSource.includes('runApiPath') && appSource.includes('/manager-action-queue/${encodeURIComponent(actionId)}/run') && appSource.includes('runManagerActionPlaybookRow') && appSource.includes('onRunRow: runManagerActionPlaybookRow') && projectDashboardManagerActionPlaybookSource.includes('onClick={() => onRunRow(row)}'), 'Project dashboard must display request body templates and execute action queue items through the backend run endpoint.');
assert(projectDashboardManagerActionRunLedgerSource.includes('Manager Action Run Ledger') && projectDashboardManagerActionRunLedgerSource.includes('manager-action-run-ledger') && projectDashboardManagerActionRunLedgerSource.includes('Run proof') && appSource.includes('backendManagerActionRuns') && appSource.includes('onOpenTimelineProof: openProjectTimelineProof') && projectDashboardManagerActionRunLedgerSource.includes('onOpenTimelineProof((run.timelineLogIds || [run.logId]).filter(Boolean))'), 'Project dashboard must expose Action Queue execution receipts with timeline proof jumps.');
assert(projectUiSource.includes('managerChangeDraft') && projectUiSource.includes('projectManagerUiStateKey') && projectUiSource.includes('managerChangeDraftFor') && projectUiSource.includes('updateManagerChangeDraft') && projectUiSource.includes('Manager Change Intake') && projectUiSource.includes('manager-change-intake-composer') && projectUiSource.includes('manager-change-composer-input') && projectUiSource.includes('manager-change-composer-mode') && projectUiSource.includes('manager-change-composer-submit') && projectUiSource.includes('submitManagerChangeIntake') && projectUiSource.includes('submitProjectMeetingMessage') && projectUiSource.includes('submitProjectChatMessage') && !projectUiSource.includes('setManagerChangeDraft(prev'), 'Project dashboard must let managers type a custom mid-project change and send it to War Room, Google Chat, or both, with the draft scoped to the active project.');
...
assert(
  projectUiSource.includes('backend-product-team-delivery-trace-source')
    && projectUiSource.includes('backend-product-team-operating-loop-source')
    && projectUiSource.includes('backend-team-collaboration-diagnostics-source')
    && projectUiSource.includes('backend-runtime-contracts-source')
    && projectUiSource.includes('backend-autonomous-cycle-consistency-source')
    && projectUiSource.includes('managerReadModelSourceLabel(operatingLoop)')
...
assert(serviceSource.includes('export function hydrateAgentProject(project = {})') && serviceSource.includes('attachWorkerRunControlsToProject(backfillProjectEventLedger(project))'), 'Project hydration must backfill legacy project event ledgers.');
...
const projectService = createAgentProjectService({
  projects: [{
    ...publishedCycleChat.project,
    initiation: { roleNegotiation, leaderElection, leaderId, approvedAt: kickoffCharter.createdAt },
  }],
  messages: [
    ...assignmentPackage.assignmentMessages.map((message) => ({ ...message, projectId })),
    ...assignmentPackage.acknowledgementMessages.map((message) => ({ ...message, projectId })),
...
assert(directAgentWorkerProbe.messages.some((message) => message.agentWorker?.targetAgentId === 'curie' && /^agent_management_turing_curie/.test(message.id || '')), 'Independent Agent worker must publish visible management check-ins for managed Agents.');
...
assert(managementResponseProbe.messages.some((message) => message.agentWorker?.targetAgentId === 'turing' && /^agent_management_response_curie_turing/.test(message.id || '')), 'Managed Agent worker pulses must respond visibly to manager check-ins.');
...
assert(firstAgentPulse.route === 'agent-work-cycle' && firstAgentPulse.messages.some((message) => message.agentWorker?.agentId === 'turing' && /^agent_work_turing_/.test(message.id || '')), 'Service per-Agent worker must publish a visible work message.');
...
assert(priorityDueAgentCycle.messages.some((message) => priorityDueAgentCycle.processed[0].managementReasons.some((reason) => message.text.includes(reason))), 'Priority-driven Agent worker messages must explain the management reason in group chat.');
...
const notDueProject = backfillProjectEventLedger({
  ...restartedApi.service.getProject(projectId),
...
try {
  ...
  assert(
    httpResponse.status === 200
      && httpBody.ready
      && httpBody.summary.proofRouteCount > 0
      && httpBody.summary.commandCenterAttentionCount === httpBody.managerCommandCenter.attentionCount
      && httpBody.managerScenarioTrail.rows.some((row) => row.id === 'leader-assignment' && row.passed),
    `HTTP server must expose the manager ready package route with command center data: status=${httpResponse.status}, ready=${httpBody.ready}, proofRoutes=${httpBody.summary?.proofRouteCount}, summaryAttention=${httpBody.summary?.commandCenterAttentionCount}, commandAttention=${httpBody.managerCommandCenter?.attentionCount}, leaderAssignment=${JSON.stringify(httpBody.managerScenarioTrail?.rows?.find((row) => row.id === 'leader-assignment'))}`,
  );
  ...
  const httpDueControl = backfillProjectEventLedger({
  ...
  const httpSchedulerDueProject = backfillProjectEventLedger({
    ...restartedDueWorkerApi.service.getProject(projectId),
  ...
  httpResponse = await fetch(`${httpRuntime.url}/workers/autonomous/tick`, {
    ...
    body: JSON.stringify({
      ...
      forceProjectIds: [`${projectId}_http_scheduler_due`],
      forceAgentRun: true,
      forceAgentProjectIds: [`${projectId}_http_scheduler_due`],
      maxAgentProjects: 1,
      maxAgentsPerProject: Math.max(1, httpSchedulerDueProject.team?.length || 1),
      submitAgentWorkArtifacts: true,
  ...
  httpBody = await httpResponse.json();
  assert(
    httpResponse.status === 200 && httpBody.result.agentProcessed.some((item) => item.projectId === `${projectId}_http_scheduler_due` && item.reviewResponseSubmission?.artifactType === 'revision-note'),
    `HTTP scheduler tick endpoint must be able to request Submitter worker linked revision responses: ${JSON.stringify((httpBody.result?.agentProcessed || []).map((item) => ({ projectId: item.projectId, agentId: item.agentId, reason: item.reason, reviewResponseArtifactType: item.reviewResponseSubmission?.artifactType, reviewArtifactType: item.reviewedSubmission?.artifactType, reviewVerdict: item.review?.verdict })))}`,
  );
