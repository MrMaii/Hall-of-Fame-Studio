...
assert(projectDashboardAgentOverviewSource.includes('kickoff-dashboard-generation-source') && appSource.includes('kickoffGenerationProvenance') && projectDashboardAgentOverviewSource.includes('Kickoff Generation Source'), 'Project dashboard must keep kickoff generation provenance visible after approval, not only during initiation.');
...
assert(timelineUiSource.includes('manager-flow-graph/missing-backend') && timelineUiSource.includes('manager-flow-source-label') && timelineUiSource.includes('manager-flow-backend-required') && timelineUiSource.includes('backendFlowGraphReady') && timelineUiSource.includes('const allowFlowFrontendFallbacks = allowLocalRuntimeFallbackForActiveProject(activeProject)') && timelineUiSource.includes('onClick={() => syncBackendManagerFlowGraph({ silent: false })}') && timelineUiSource.includes('disabled={!backendCommandAvailable || backendStation.loading}') && timelineUiSource.includes('onClick={() => confirmManagerFlowNode(selectedNode.id, true)}') && timelineUiSource.includes('onClick={() => confirmManagerFlowNode(selectedNode.id, false)}'), 'Manager Flow Graph must suppress frontend fallback rows for backend-synced real projects and keep Sync/Confirm/Supersede backend-attemptable from the configured command target rather than a stale frontend online status label.');
