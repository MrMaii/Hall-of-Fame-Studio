...
const ACCEPTANCE_IS_PRIVATE_PILOT_STAGE = ACCEPTANCE_STAGE.startsWith('private-pilot');
const ACCEPTANCE_IS_PRODUCTION_STAGE = ACCEPTANCE_STAGE.startsWith('production-');
const ACCEPTANCE_RUNS_DEEP_CORE_PREFLIGHT = !(ACCEPTANCE_IS_PRIVATE_PILOT_STAGE || ACCEPTANCE_IS_PRODUCTION_STAGE);
const ACCEPTANCE_RUNS_OPERATIONS_PREFLIGHT = ![
  'private-pilot-launch-handoff',
].includes(ACCEPTANCE_STAGE);
const ACCEPTANCE_STAGE_STOPS = {
  ...
  'private-pilot-ui-launch-approval-prep': 'private-pilot launch approval UI prep ready',
...
const appEntrySource = readFileSync(new URL('../src/App.jsx', import.meta.url), 'utf8');
const uiComponentDirectories = [
  'common',
  'localRuntime',
  'meeting',
  'navigation',
...
const uiComponentSource = uiComponentDirectories
  .flatMap((directory) => readdirSync(new URL(`../src/${directory}/`, import.meta.url))
    .filter((name) => /\.jsx$/.test(name))
    .map((name) => readFileSync(new URL(`../src/${directory}/${name}`, import.meta.url), 'utf8')))
  .join('\n');
...
progress('static source fixtures loaded');
...
assert(appSource.includes('submission-review-failed') && appSource.includes('No local review receipt was created.') && appSource.includes('Review write failed:') && appSource.includes("reviewReceipt?.action === 'submission-review-failed'"), 'Reviewer composer failures must replace pending receipts with a visible failed state instead of displaying a fake or stuck review receipt.');
assert(agentAutonomousActionQueueUiSource.includes('Agent Action Failed') && agentAutonomousActionQueueUiSource.includes('No local run receipt was created:') && appSource.includes('Intent Run Failed') && appSource.includes('Action failed:') && appSource.includes('No local intent receipt was created.') && appSource.includes('No local operator receipt was created.') && appSource.includes('agentAutonomousActionRun: null') && appSource.includes('collaborationIntentRun: null'), 'A-side continuation controls must clear stale receipts and show failed non-proof states when backend run routes fail.');
...
assert(
  ...
  'Settings Integrations route-backed cards must show sync-required/row-missing when the capability contract is absent instead of implying missing backend APIs.',
);
...
assert(appSource.includes('backend-submission-review-workflow-proof-rows') && appSource.includes('backend-submission-review-workflow-route-${row.id}') && appSource.includes('backend-submission-review-workflow-chat-proof-${row.id}') && appSource.includes('backend-submission-review-workflow-timeline-proof-${row.id}') && appSource.includes('Review: {row.route') && appSource.includes('Response: {(row.responseRoutes || []).slice(0, 2).join'), 'Submission Review Workflow UI must expose per-review backend routes plus chat/timeline proof exits.');
...
assert(appSource.includes('backend-product-team-mission-handoff-execution') && appSource.includes('backendMissionHandoffExecution') && appSource.includes('product-team-customer-agent-handoff-execution/v1') && appSource.includes('backend-product-team-mission-run-handoff-intent') && appSource.includes('runCollaborationIntentQueueRow(backendMissionHandoffIntentRow)') && appSource.includes('backend-product-team-mission-handoff-latest-output') && appSource.includes('backend-product-team-mission-handoff-chat-proof') && appSource.includes('backend-product-team-mission-handoff-timeline-proof'), 'Manager Dashboard Mission Runner card must expose A-side handoff execution proof and run C/A handoff through the backend Collaboration Intent Queue.');
assert(projectDashboardManagerActionRunLedgerSource.includes('manager-action-run-output') && projectDashboardManagerActionRunLedgerSource.includes('Manager Action Output Nodes') && projectDashboardManagerActionRunLedgerSource.includes('manager-action-output-${row.id}') && projectDashboardManagerActionRunLedgerSource.includes('manager-action-output-chat-proof'), 'Manager UI must expose Manager Action Queue run outputs as backend result nodes, not just ledger receipts.');
...
assert(agentAutonomousActionQueueUiSource.includes('backend-agent-autonomous-action-run-output') && agentAutonomousActionQueueUiSource.includes('Agent Action Output Nodes') && agentAutonomousActionQueueUiSource.includes('backend-agent-autonomous-action-output-${row.id}') && agentAutonomousActionQueueUiSource.includes('agent-autonomous-action-output-route-${row.id}') && agentAutonomousActionQueueUiSource.includes("route: output.workSubmission.route") && agentAutonomousActionQueueUiSource.includes("route: output.evidenceSearch.route") && agentAutonomousActionQueueUiSource.includes("Route: {row.route || 'route pending'} / Event: {row.eventId || 'missing'}") && agentAutonomousActionQueueUiSource.includes('agent-autonomous-action-output-chat-proof'), 'Manager UI must expose direct Agent Autonomous Action Queue outputs as backend product nodes with backend routes/events, not just an action receipt.');
...
assert(appSource.includes('backend-manager-submission-review-row-${row.id}') && appSource.includes('backend-manager-submission-review-route-${row.id}') && appSource.includes('backend-manager-submission-review-chat-proof-${row.id}') && appSource.includes('backend-manager-submission-review-timeline-proof-${row.id}') && appSource.includes('managerReviewChatProofIds') && appSource.includes('managerReviewTimelineIds'), 'Manager Dashboard submission-review rows must expose review/submission routes plus chat/timeline proof exits.');
assert(appSource.includes('selectedNodeSubmissionRouteCandidates') && appSource.includes('selectedNodeSubmissionRoute') && appSource.includes('selectedNodeSubmissionId') && appSource.includes('selectedNodeHasSubmissionRecord') && appSource.includes('openSelectedNodeSubmissionRecord') && appSource.includes('backend-manager-submission-row-${row.id}') && appSource.includes('manager-flow-selected-submission-route') && appSource.includes('manager-flow-selected-submission-record-open') && appSource.includes('manager-flow-selected-submission-route-evidence') && appSource.includes('manager-flow-selected-submission-record-open-evidence'), 'Manager Flow Graph selected submission/final-deliverable node detail must expose a direct backend submission-record route and dashboard jump.');
...
assert(appSource.includes('const firstBackendRoute =') && appSource.includes('collaborationSourceReviewRows') && appSource.includes('message.evidenceSearchRoute') && appSource.includes('message.evidenceSourceReviewRoute') && appSource.includes('message.submissionReviewRoute') && appSource.includes('message.submissionRoute') && appSource.includes('evidence-source-review-${message.reviewId}'), 'Group Chat collaboration cards and transcript index rows must prefer backend-provided resource routes for submission, evidence-search, source-review, and submission-review nodes.');
...
assert(appSource.includes("['mvpReadiness', readRoutes.mvpReadinessRoute, 'mvpReadiness', 5200]") && appSource.includes("workflowKey: 'mvpReadiness'") && appSource.includes('MVP readiness action receipt routes refreshed'), 'Manager UI must refresh MVP readiness operator action proof routes through the shared receipt refresh path.');
...
progress('static UI/backend source preflight completed');
...
progress('acceptance temp directory ready');
const preserveAcceptanceTmp = process.env.HOFS_PRODUCT_TEAM_PRESERVE_TMP === '1'
  || process.env.HOFS_KEEP_PRODUCT_TEAM_TMP === '1';
let acceptanceTmpCleaned = false;
function cleanupAcceptanceTmp() {
  if (preserveAcceptanceTmp) return;
  if (acceptanceTmpCleaned) return;
  acceptanceTmpCleaned = true;
  rmSync(root, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 });
}
process.on('exit', cleanupAcceptanceTmp);
for (const signal of ['SIGINT', 'SIGTERM', 'SIGHUP']) {
  process.once(signal, () => {
    cleanupAcceptanceTmp();
...
progress('secret vault direct preflight completed');
...
progress('secret vault route preflight completed');
...
if (ACCEPTANCE_RUNS_DEEP_CORE_PREFLIGHT) {
const missionRunnerApi = createAgentProjectApi({
...
progress('mission runner startup receipt checked');
...
progress('mission runner intent queue checked');
...
progress('mission runner handoff intent run checked');
missionResponse = missionRunnerApi.handle({ method: 'GET', path: `/projects/${missionProjectId}/collaboration-intent-queue` });
...
progress('mission runner agent intent run checked');
missionResponse = missionRunnerApi.handle({ method: 'GET', path: `/projects/${missionProjectId}/collaboration-intent-queue` });
...
progress('mission runner group-chat intent run checked');
...
progress('mission runner manager intent run checked');
...
progress('reused kickoff mission runner backend chain ready');
  ...
  progress('deep Mission Runner preflight skipped for staged private-pilot or production gate');
}
...
progress('mvp readiness target execution chain completed');
  ...
  progress('deep MVP readiness target execution skipped for staged private-pilot or production gate');
}
...
assert(response.body.artifactDraft.artifactDraftQuality?.schemaVersion === 'artifact-draft-quality/v1'
  && response.body.artifactDraft.artifactDraftQuality.generationMode === 'explicit-local-template'
  && response.body.artifactDraft.artifactDraftQuality.status === 'template-review-required'
  && response.body.artifactDraft.artifactDraftQuality.structurallyReadyForReview === true
  && response.body.artifactDraft.artifactDraftQuality.readyForLocalPilot === false
  && response.body.artifactDraft.artifactDraftQuality.readyForProduction === false,
'Generated local template drafts must pass structural gates while remaining review-required without pilot or production overclaim.');
...
response = api.handle({
  method: 'POST',
  path: `/projects/${projectId}/submissions/${encodeURIComponent(generatedDraftSubmission.id)}/reviews`,
  body: {
    reviewerAgentId: 'curie',
    verdict: 'accepted',
    comments: 'Local template structure and proof context were reviewed before pilot use.',
    now: '2026-06-01T11:12:30.000Z',
  },
});
assert(response.status === 200 && response.body.review?.verdict === 'accepted', 'Local template artifact drafts must receive a real backend Reviewer decision before the quality audit can accept them.');
...
progress('post-artifact submissions checks starting');
...
progress('post-artifact submissions checks completed');

progress('post-artifact evidence checks starting');
...
for (const [index, item] of initialEvidenceSourceReviewWorkflow.reviewItems.entries()) {
  response = api.handle({
    ...
    body: {
      ...
      includeReadModels: ACCEPTANCE_RUNS_DEEP_CORE_PREFLIGHT,
    },
  });
  ...
  if (ACCEPTANCE_RUNS_DEEP_CORE_PREFLIGHT) {
    ...
    assert(response.body.readModels?.included === false && response.body.readModels?.evidenceSourceReviewWorkflowRoute?.endsWith('/evidence-source-review-workflow'), 'Staged source review decisions must support deferred workflow refresh.');
  }
...
progress('post-artifact evidence checks completed');

response = api.handle({ method: 'GET', path: `/projects/${projectId}/submission-reviews` });
assert(response.status === 200 && response.body.submissionReviews.length === 3, 'Project API must list the change-request, final acceptance, and local-template acceptance reviews.');
...
progress('manager dashboard read starting');
response = api.handle({ method: 'GET', path: `/projects/${projectId}/manager-dashboard` });
progress('manager dashboard read completed');
...
assert(response.body.submissions.draftQualityReadyCount === 1 && response.body.submissions.modelDraftHumanReviewRequiredCount === 1, 'Manager Dashboard must count only provider-generated draft quality readiness while retaining model human-review requirements.');
...
assert(response.body.submissions.rows.some((row) => row.id === generatedDraftSubmission.id && row.artifactDraftQuality?.readyForLocalPilot === false && row.artifactDraftQuality?.structurallyReadyForReview === true && row.artifactDraftQualityStatus === 'template-review-required'), 'Manager Dashboard rows must expose local template drafts as structurally ready but still review-required.');
...
assert(response.body.submissionReviews.acceptedCount === 2 && response.body.submissionReviews.changesRequestedCount === 1, 'Manager Dashboard must expose final-deliverable and local-template acceptances plus the requested-change review.');
...
assert(response.body.productionProviderControlReceiptWorkflow.missingControlIds.includes('real-provider-eval-run') && response.body.productionProviderControlReceiptWorkflow.missingControlIds.includes('managed-provider-eval-storage'), 'Production provider control receipt workflow must expose real eval and managed storage as missing controls.');
  ...
  progress('production control workflow preflight skipped for staged private-pilot or production gate');
}

if (ACCEPTANCE_RUNS_OPERATIONS_PREFLIGHT) {
...
assert(response.body.securityAccessAudit.stream?.hashChainReady === true, 'Security access audit must expose a verified backend audit hash chain.');
  ...
  progress('enforced access route preflight skipped for launch-handoff gate');
}
...
assert(response.body.persistenceSnapshot.recordsByTable.agent_submissions.some((record) => record.data.id === generatedDraftSubmission.id && record.data.isGeneratedDraft === true && record.data.artifactDraftSource === 'local-artifact-draft-generator' && record.data.artifactDraftChecksum && record.data.artifactDraftQualityStatus === 'template-review-required'), 'Production persistence snapshot must preserve local template draft provenance and review-required quality fields.');
...
assert(submissionNodes.every((node) => node.submissionId && node.submissionRoute === `/projects/${projectId}/submissions/${encodeURIComponent(node.submissionId)}` && node.route === node.submissionRoute && node.attachments.some((attachment) => attachment.submissionId === node.submissionId && attachment.submissionRoute === node.submissionRoute)), 'Every explicit submission node must expose a backend submissionId/submissionRoute on the node and artifact attachment.');
...
assert(evidenceNodes.every((node) => node.evidenceSearchId && node.evidenceSearchRoute === `/projects/${projectId}/evidence-searches/${encodeURIComponent(node.evidenceSearchId)}` && node.route === node.evidenceSearchRoute && node.attachments.some((attachment) => attachment.evidenceSearchId === node.evidenceSearchId && attachment.evidenceSearchRoute === node.evidenceSearchRoute)), 'Manager Flow Graph evidence-search nodes must expose explicit backend evidenceSearchId/evidenceSearchRoute fields on nodes and attachments.');
...
assert(evidenceSourceReviewNodes.every((node) => node.evidenceSourceReviewId && node.evidenceSourceReviewRoute === `/projects/${projectId}/evidence-source-review-workflow#${encodeURIComponent(node.evidenceSourceReviewId)}` && node.route === node.evidenceSourceReviewRoute && node.evidenceSearchId === evidenceSearch.id && node.evidenceSearchRoute === `/projects/${projectId}/evidence-searches/${encodeURIComponent(evidenceSearch.id)}` && node.attachments.some((attachment) => attachment.evidenceSourceReviewId === node.evidenceSourceReviewId && attachment.evidenceSourceReviewRoute === node.evidenceSourceReviewRoute)), 'Manager Flow Graph evidence-source-review nodes must expose explicit backend source-review and evidence-search resource fields.');
...
assert(reviewNodes.length === 3 && reviewNodes.every((node) => node.proofIds.length && node.timelineLogIds.length && node.eventIds.length && node.attachments.length), 'Manager Flow Graph must include proofed change-request, final acceptance, and local-template acceptance review nodes.');
assert(reviewNodes.every((node) => node.submissionReviewId && node.submissionReviewRoute === `/projects/${projectId}/submission-reviews/${encodeURIComponent(node.submissionReviewId)}` && node.route === node.submissionReviewRoute && node.submissionId && node.submissionRoute === `/projects/${projectId}/submissions/${encodeURIComponent(node.submissionId)}` && node.attachments.some((attachment) => attachment.submissionReviewId === node.submissionReviewId && attachment.submissionReviewRoute === node.submissionReviewRoute)), 'Manager Flow Graph submission-review nodes must expose explicit backend submissionReviewId/submissionReviewRoute fields on nodes and attachments.');
...
assert(response.body.submissionRoutes.every((route) => route.id && route.submissionId === route.id && route.submissionRoute === `/projects/${projectId}/submissions/${encodeURIComponent(route.submissionId)}` && route.apiPath === route.submissionRoute), 'Readiness Proof Map submission routes must expose explicit backend submissionId/submissionRoute fields.');
...
assert(response.body.evidenceSearchRoutes.every((route) => route.id && route.evidenceSearchId === route.id && route.evidenceSearchRoute === `/projects/${projectId}/evidence-searches/${encodeURIComponent(route.evidenceSearchId)}` && route.apiPath === route.evidenceSearchRoute), 'Readiness Proof Map evidence-search routes must expose explicit backend evidenceSearchId/evidenceSearchRoute fields.');
...
assert(response.body.evidenceSourceReviewRoutes.every((route) => route.id && route.evidenceSourceReviewId === route.id && route.evidenceSourceReviewRoute === `/projects/${projectId}/evidence-source-review-workflow#${encodeURIComponent(route.evidenceSourceReviewId)}` && route.apiPath === route.evidenceSourceReviewRoute && route.evidenceSearchId === evidenceSearch.id && route.evidenceSearchRoute === `/projects/${projectId}/evidence-searches/${encodeURIComponent(evidenceSearch.id)}`), 'Readiness Proof Map evidence-source-review routes must expose explicit backend source-review and evidence-search resource fields.');
...
assert(response.body.submissionReviewSummary.acceptedCount === 2 && response.body.submissionReviewRoutes.some((route) => route.verdict === 'changes-requested'), 'Readiness Proof Map must expose both acceptance reviews and the requested-change route.');
assert(response.body.submissionReviewRoutes.every((route) => route.id && route.submissionReviewId === route.id && route.submissionReviewRoute === `/projects/${projectId}/submission-reviews/${encodeURIComponent(route.submissionReviewId)}` && route.apiPath === route.submissionReviewRoute && route.submissionId && route.submissionRoute === `/projects/${projectId}/submissions/${encodeURIComponent(route.submissionId)}`), 'Readiness Proof Map submission-review routes must expose explicit backend review and submission resource fields.');
...
assert(response.body.messages.some((message) => message.type === 'submission' && message.submissionId === finalSubmissionId && message.submissionRoute === `/projects/${projectId}/submissions/${encodeURIComponent(finalSubmissionId)}` && message.resourceRoute === message.submissionRoute), 'Group Chat submission messages must expose backend submission routes for collaboration cards.');
assert(response.body.messages.some((message) => message.type === 'evidence-search' && message.evidenceSearchId === evidenceSearch.id && message.evidenceSearchRoute === `/projects/${projectId}/evidence-searches/${encodeURIComponent(evidenceSearch.id)}` && message.resourceRoute === message.evidenceSearchRoute), 'Group Chat evidence-search messages must expose backend evidence-search routes for collaboration cards.');
const sourceReviewTranscriptMessages = response.body.messages.filter((message) => message.type === 'evidence-source-review' && message.evidenceSearchId === evidenceSearch.id);
assert(sourceReviewTranscriptMessages.length === 3 && sourceReviewTranscriptMessages.every((message) => message.evidenceSourceReviewRoute === `/projects/${projectId}/evidence-source-review-workflow#${encodeURIComponent(message.reviewId)}` && message.evidenceSearchRoute === `/projects/${projectId}/evidence-searches/${encodeURIComponent(evidenceSearch.id)}` && message.resourceRoute === message.evidenceSourceReviewRoute), 'Group Chat evidence-source-review messages must expose backend source-review and evidence-search routes for collaboration cards.');
const finalReviewId = reviewsByVerdict.get('accepted')?.id;
assert(response.body.messages.some((message) => message.type === 'submission-review' && message.reviewId === finalReviewId && message.submissionReviewRoute === `/projects/${projectId}/submission-reviews/${encodeURIComponent(finalReviewId)}` && message.submissionRoute === `/projects/${projectId}/submissions/${encodeURIComponent(finalSubmissionId)}` && message.resourceRoute === message.submissionReviewRoute), 'Group Chat submission-review messages must expose backend review and submission routes for collaboration cards.');
...
assert(response.body.eventLedger.some((event) => event.type === 'provider-usage'), 'Event ledger must include provider usage audit events.');
  ...
  progress('deep audit, persistence, worker, dashboard, and proof preflight skipped for launch-handoff gate');
}
...
assert(httpBody.productTeamOperatingLoop?.proofLoop?.proofRouteCount > 0 && httpBody.productTeamOperatingLoop?.proofLoop?.eventIds?.length > 0, 'HTTP product-team operating loop must keep proof routes and event evidence attached.');
  ...
  progress('local HTTP runtime preflight skipped for staged private-pilot or production gate');
}
...
try {
  ...
  progress('signed access and replay preflight skipped for staged private-pilot gate');
}
...
if (ACCEPTANCE_STAGE === 'private-pilot-launch-handoff') {
  const handoffLaunchApprovalPath = `/projects/${projectId}/launch-approvals`;
  response = membershipApi.handle({
    method: 'POST',
    path: handoffLaunchApprovalPath,
    headers: signedHeadersFor({
      method: 'POST',
      path: handoffLaunchApprovalPath,
      role: 'manager',
    ...
    body: {
      ...
      reason: 'Manager approves the private-pilot launch handoff after the generic product-team proof chain is present.',
      linkedAuditChecksum: 'private-pilot-launch-handoff-local-proof',
      now: '2026-06-01T11:46:00.000Z',
    },
  });
  assert(response.status === 200 && response.body.launchApproval?.schemaVersion === 'launch-approval/v1', 'Private-pilot launch handoff must record the manager launch approval.');
  assert(response.body.launchApprovalWorkflow?.readyForPrivatePilot === false, 'Private-pilot launch handoff must still require security approval after manager approval.');
  response = membershipApi.handle({
    method: 'POST',
    path: handoffLaunchApprovalPath,
    headers: signedHeadersFor({
    ...
    body: {
      ...
      reason: 'Security approves the private-pilot launch handoff while production controls remain blocked.',
      linkedAuditChecksum: 'private-pilot-launch-handoff-local-proof',
      now: '2026-06-01T11:47:00.000Z',
    },
  });
  assert(response.status === 200 && response.body.launchApproval?.schemaVersion === 'launch-approval/v1', 'Private-pilot launch handoff must record the security launch approval.');
  assert(response.body.launchApprovalWorkflow?.readyForPrivatePilot === true, 'Private-pilot launch handoff approvals must make the launch approval workflow private-pilot ready.');
  assert(response.body.launchApprovalWorkflow?.readyForProduction === false, 'Private-pilot launch handoff approvals must not claim production readiness.');
  response = membershipApi.handle({
    method: 'GET',
    path: handoffLaunchApprovalPath,
    headers: signedHeadersFor({
  ...
  assert(response.status === 200 && response.body.launchApprovalWorkflow?.rows?.length >= 2, 'Private-pilot launch handoff must read back the launch approval receipts.');
  progress('private-pilot launch approval record ready');
}

let managerIdentitySessionToken = 'private-pilot-stage-identity-session-not-created';
let managerIdentitySessionId = 'private-pilot-stage-identity-session-id-not-created';
let activeSecurityIdentitySessionToken = 'private-pilot-stage-active-security-session-not-created';
let activeSecurityIdentitySessionId = 'private-pilot-stage-active-security-session-id-not-created';
...
managerIdentitySessionToken = response.body.token;
managerIdentitySessionId = response.body.identitySession.id;
...
activeSecurityIdentitySessionToken = response.body.token;
activeSecurityIdentitySessionId = response.body.identitySession.id;
...
assert(response.body.persistenceAdapterDryRun.readModelProbe.expectedReadModels.every((name) => response.body.persistenceAdapterDryRun.readModelProbe.checkpointNames.includes(name)), 'Managed persistence adapter dry-run must prove read-model checkpoint parity.');
  ...
  progress('membership identity-session and persistence adapter preflight skipped for launch-handoff gate');
}
...
progress('private-pilot launch approval UI prep ready');
progress('private-pilot launch approval record ready');
response = membershipApi.handle({
  method: 'POST',
...
for (const secret of [FAKE_SEARCH_SECRET, FAKE_MODEL_SECRET, FAKE_SOURCE_SECRET, ACCESS_SIGNING_SECRET, MANAGED_PRODUCTION_ATTESTATION_SIGNING_SECRET, managerIdentitySessionToken, activeSecurityIdentitySessionToken]) {
...
assert(response.body.productionOperationsReadiness.managedProductionEvidence?.schemaVersion === 'production-operations-managed-production-evidence/v1', 'Production operations readiness must expose a strict managed-production evidence summary.');
assert(response.body.productionOperationsReadiness.managedProductionEvidence.readyForManagedProductionOperationsEvidence === false && response.body.productionOperationsReadiness.managedProductionEvidence.summary?.managedProductionControlCount === 0, 'Production operations readiness must keep managed-production evidence blocked before signed control-plane attestations.');
...
assert(response.body.productionOperationsReadiness?.managedProductionEvidence?.readyForManagedProductionOperationsEvidence === false && response.body.productionOperationsReadiness?.managedProductionEvidence?.summary?.localRehearsalControlCount === productionOperationsControlIds.length, 'Production operations readiness must still show local-rehearsal operations evidence after local/test receipts.');
...
for (const batch of managedProductionReceiptBatches) {
  progress(`unattested managed production receipt ${batch.prefix} starting`);
  response = membershipApi.handle({
    method: 'POST',
    path: batch.path,
    headers: signedHeadersFor({
    ...
    body: {
      ...
      reason: 'Record an intentionally unattested managed-production claim; it must not pass evidence integrity.',
      now: new Date(Date.parse(batch.now) - 30_000).toISOString(),
      includeReadModels: false,
      controls: batch.controlIds.map((controlId) => ({
        controlId,
        status: 'verified',
        evidenceId: `${batch.prefix}_${controlId}_unattested_receipt`,
        evidenceRoute: `${batch.routePrefix}/${controlId}/unattested`,
        evidenceChecksum: `${batch.prefix}_${controlId}_unattested_checksum`,
        evidenceEnvironment: 'managed-production',
        attestationId: `${batch.prefix}_${controlId}_unsigned_attestation`,
        attestationRoute: `${batch.routePrefix}/${controlId}/unsigned-attestation`,
        attestationChecksum: `${batch.prefix}_${controlId}_unsigned_attestation_checksum`,
        attestationProvider: 'managed-production-control-plane',
        attestationKind: 'managed-control-plane-attestation',
        completedAt: new Date(Date.parse(batch.now) - 45_000).toISOString(),
        ownerRole: batch.actorRole,
        detail: `Claimed ${controlId} as managed-production with unsigned attestation.`,
      })),
    },
  });
  assert(response.status === 200, `Unattested managed-production receipt batch must be accepted but not treated as production evidence for ${batch.path}.`);
  progress(`unattested managed production receipt ${batch.prefix} completed`);
}
...
assert(response.status === 200 && response.body.productionEvidenceIntegrityAudit?.schemaVersion === 'production-evidence-integrity-audit/v1', 'Production evidence integrity audit must remain readable after unattested managed-production receipts.');
assert(response.body.productionEvidenceIntegrityAudit.summary?.managedProductionControlCount === 0, 'Production evidence integrity audit must reject managed-production claims without attestation.');
assert(response.body.productionEvidenceIntegrityAudit.summary?.externalUnattestedControlCount === totalProductionControlCount, 'Production evidence integrity audit must classify unattested managed-production claims as external-unattested.');
assert(response.body.productionEvidenceIntegrityAudit.readyForManagedProductionEvidence === false && response.body.productionEvidenceIntegrityAudit.readyForProduction === false, 'Unattested managed-production claims must keep public production blocked.');
assert(response.body.productionEvidenceIntegrityAudit.rows?.every((row) => row.evidenceEnvironment === 'managed-production' && row.evidenceTier === 'external-unattested' && row.attestationReady === false && row.attestationSignatureReady === false && row.attestationFailureReason === 'managed-production-attestation-signature-missing'), 'Production evidence integrity rows must expose missing signed attestation for unattested managed-production claims.');
    ...
    body: {
      ...
      controls: batch.controlIds.map((controlId) => {
        const control = {
          ...
          evidenceEnvironment: 'managed-production',
          attestationId: `${batch.prefix}_${controlId}_attestation`,
          attestationRoute: `${batch.routePrefix}/${controlId}/attestation`,
          attestationChecksum: `${batch.prefix}_${controlId}_attestation_checksum`,
          attestationProvider: 'managed-production-control-plane',
          attestationKind: 'managed-control-plane-attestation',
          completedAt: batch.now,
          ownerRole: batch.actorRole,
          detail: `Verified ${controlId} with explicit managed-production evidence and signed attestation.`,
        };
        return {
          ...control,
...
assert(response.body.productionEvidenceIntegrityAudit.rows?.every((row) => row.evidenceTier === 'managed-production' && row.evidenceEnvironment === 'managed-production' && row.attestationReady === true && row.attestationSignatureReady === true && row.attestationChecksum && row.attestationSignature), 'Production evidence integrity audit rows must preserve managed-production environment and signed attestation evidence.');
...
assert(response.body.productionLaunchControlCenter.controlRows?.some((row) => row.id === 'public-production-startup-readiness' && row.ready === false && row.apiPath === '/public-production-startup-readiness' && row.missing?.length), 'Production launch control center must include the global public-production startup blocker before public launch.');
assert(response.body.productionLaunchControlCenter.summary?.publicProductionStartupReady === false && response.body.productionLaunchControlCenter.summary?.publicProductionStartupFailedGateCount > 0, 'Production launch control center must summarize public-production startup readiness as blocked in local/rehearsal mode.');
...
assert(response.body.productionLaunchControlCenter.stageRows?.some((row) => row.id === 'public-production-startup' && row.ready === false && row.apiPath === '/public-production-startup-readiness'), 'Production launch control center stage rows must preserve the global public-production startup blocker.');
...
for (const secret of [FAKE_SEARCH_SECRET, FAKE_MODEL_SECRET, FAKE_SOURCE_SECRET, ACCESS_SIGNING_SECRET, MANAGED_PRODUCTION_ATTESTATION_SIGNING_SECRET]) {
